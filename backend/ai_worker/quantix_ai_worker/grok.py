...
def _safe_failure_message(reason, *, operation):
    if operation == "check":
        if reason == "max_turns":
            return (
                f"Grok reached the {GROK_CHECK_MAX_ROUNDS}-round connection check limit before completing "
...
def update_usage(usage, event, expected_model, request_limit):
    ...
    usage.update(
        provider_usage_is_incomplete=incomplete if type(incomplete) is bool else None,
    ...
    if not isinstance(aggregate, dict):
        ...
        if wrong_model or exceeded_turns:
            raise RuntimeUnavailable(
                "Grok reported work outside the approved model or turn allowance. Its result was withheld."
            )
        return
    names = (
        "input_tokens",
    ...
    usage["usage_complete"] = bool(
        ...
        and turns > 0
        and incomplete is not True
        and one_approved_model
        and not exceeded_turns
    )
    if wrong_model:
        raise RuntimeUnavailable(
            "Grok reported a different or additional model. The result was withheld because that model was not approved for this request."
        )
    if exceeded_turns:
        raise RuntimeUnavailable(
            "Grok exceeded this run's approved model-turn allowance. Its result was withheld."
        )


def _tool_name(value):
    return (
        value if isinstance(value, str) else value.get("name") if isinstance(value, dict) else None
    )
...
async def execute_grok(
    ...
    *,
    ...
    if (
        connection.get("auth_type") != "client_login"
        or connection.get("billing") != "subscription"
        or credentials
    ):
        raise RuntimeUnavailable(
            "Use the original Grok Build sign-in for this subscription connection. API-key billing requires its separate xAI API connection."
        )
    if route.get("web_search"):
        raise RuntimeUnavailable(
            "This Grok Build connection exposes Tender tools only. Use an approved research connection for online sources."
        )
    ...
    usage = runtime_usage(
        ...
        provider_cost_is_partial=None,
        provider_usage_is_incomplete=None,
    ...
    async def waiting():
        ...
        await context.control.event(
            "runtime_waiting",
            "Waiting for this Grok account's current operation to finish.",
    ...
    try:
        async with account_lock(home, on_wait=waiting, before_release=release):
            ...
            async with acp_session(home, connection) as (client, initialized):
                ...
                selected = next(
                    (model for model in models if model["model_id"] == route["model_id"]), None
                )
                if selected is None:
                    raise RuntimeUnavailable(
                        "The selected model is no longer available to this Grok account. Refresh its model list before retrying."
                    )
                if (
                    route.get("reasoning")
                    and route["reasoning"] not in selected["capabilities"]["reasoning"]
                ):
                    raise RuntimeUnavailable(
                        "Grok did not advertise the approved reasoning setting for this model. Review the model settings."
                    )
                record(
                    "grok_phase",
                    phase="preflight",
            ...
            async with bridge.serve():
                ...
                if operation != "check":
                    await public_events.request_activity(
                        "prepared",
                        payload={
                            "instructions": profile_content,
                ...
                async with asyncio.timeout(timeout):
                    process = await spawn(
                        command, work, environment(home, bridge=bridge), limit=16 * 1024 * 1024
                    )
                    record(
                        "grok_process",
                        phase="process",
                    ...
                    while line := await process.stdout.readline():
                        ...
                        if total > 64 * 1024 * 1024:
                            raise RuntimeUnavailable(
                                "Grok exceeded the bounded output stream for this Tender run."
                            )
                        try:
                            event = json.loads(line)
                        except (ValueError, UnicodeDecodeError):
                            raise RuntimeUnavailable(
                                "Grok returned an unsupported structured stream. Its result was withheld."
                            ) from None
                        if not isinstance(event, dict) or not isinstance(event.get("type"), str):
                            raise RuntimeUnavailable("Grok returned an invalid structured event.")
                        if terminal is not None:
                            raise RuntimeUnavailable(
                                "Grok emitted content after its terminal result. Its result was withheld."
                            )
                        kind = event["type"]
                        if kind == "text" and operation != "check":
                            await public_events.text(
                                event.get("data"), item_id=f"response-{usage['requests'] + 1}"
                            )
                        if kind == "usage":
                            identity = event.get("messageId")
                            if identity is not None and (
                                not isinstance(identity, str) or len(identity) > 500
                            ):
                            ...
                            if identity is None or identity not in response_ids:
                                ...
                                await context.control.event(
                                    ...
                                    {
                                        "round": usage.get("requests"),
                                        "request_id": public_events.request_id,
                            ...
                            if usage["requests"] > requests:
                                raise RuntimeUnavailable(
                                    "Grok exceeded the approved model-turn allowance. Its result was withheld."
                                )
                        if kind == "available_commands":
                            ...
                            if isinstance(tools, list):
                                ...
                                if any(_tool_name(tool) not in allowed for tool in tools):
                                    raise RuntimeUnavailable(
                                        "Grok advertised tools outside this Tender's approved tool set."
                                    )
                        if kind == "tool_call":
                            ...
                            if name not in {
                                ...
                                *("quantix__" + item for item in bridge.names),
                                ...
                                raise RuntimeUnavailable(
                                    "Grok attempted a tool outside the approved Tender tool set."
                                )
                            if operation != "check":
                                await context.control.event(
                                    ...
                                    "Original client tool activity.",
                                    {
                                        "tool": name,
                                        "phase": "started",
                                        "provider_call_id": event.get("toolCallId"),
                                        "request_id": public_events.request_id,
                                    },
                                )
                        if kind.startswith("auto_compact"):
                            ...
                            raise RuntimeUnavailable(
                                "Grok reached a context-compaction boundary. Split the engineering work before retrying; auxiliary inference is not approved for this run."
                            )
                        if kind in {"error", "max_turns_reached"}:
                            error_details = (
                                classify_provider_error(event)
                                if kind == "error"
                                else {"category": "max_turns", "status_code": None}
                            )
                            record(
                                "grok_terminal",
                                phase="inference",
                    ...
                    if failed:
                        raise RuntimeUnavailable(
                            _safe_failure_message(
                                failure_reason or "provider_error", operation=operation
                            )
                        )
                    if terminal is None:
                        record(
                        ...
                        raise RuntimeConnectionFailure(
                            _safe_failure_message("missing_terminal", operation=operation)
                            + " Any unreported usage remains uncertain."
                        )
                    if process.returncode != 0:
                        raise RuntimeConnectionFailure(
                            "Grok's provider process ended unexpectedly. Retry the operation; any unreported usage remains uncertain."
                        )
                    ...
                    if terminal.get("stopReason") != "end_turn":
                        raise RuntimeUnavailable(
                            "Grok stopped before the complete engineering proposal was ready."
                        )
                    if not model_identity_matches(route["model_id"], usage.get("actual_model")):
                        raise RuntimeUnavailable(
                            "Grok did not establish the exact approved model identity. The result was withheld."
                        )
                    try:
                        ...
                        if operation == "check":
                            raise RuntimeUnavailable(
                                "Grok completed without a validated structured connection-check submission. Retry the access check."
                            ) from None
                        raise RuntimeUnavailable(
                            "Grok completed without a validated structured engineering submission. Retry this operation."
                        ) from None
                    ...
                    if binding and usage.get("session_id"):
                        await context.control.event(
                            "runtime_session",
                            "Original client session observed.",
                            {
                    ...
                    await context.control.event(
                        ...
                        "The original client reported its selected model.",
                        {
                            "connection_id": connection["id"],
                    ...
                    if operation != "check":
                        await public_events.request_activity(
                            "completed", payload={"usage": usage, "output": result}
                        )
