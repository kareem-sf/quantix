...
def session_binding(context, connection, route):
    ...
    if binding is None:
        return None
    if (not isinstance(binding, dict) or not re.fullmatch(r"[a-f0-9]{32}", str(binding.get("id", "")))
        or binding.get("connection_id") != connection.get("id") or binding.get("connection_revision") != connection.get("revision")
        or binding.get("protocol") != connection.get("protocol") or binding.get("model_id") != route.get("model_id")
        ...
        raise RuntimeUnavailable("The original-client session binding does not match this operation.")
    ...
    if identifier is not None:
        ...
        if connection["protocol"] == "grok_build":
            try:
                ...
                raise RuntimeUnavailable("Grok can safely resume only an exact native UUID, never a session title. Start a new session.") from None
    return binding


def work_directory(context, binding, fallback_id):
    root = context.account_home.resolve()
    work = root / "work" / ("sessions" if binding else context.operation_id) / (binding["id"] if binding else fallback_id)
    work = work.resolve()
    if not work.is_relative_to(root):
        raise RuntimeUnavailable("The original-client working directory is outside its private account home.")
...
class ClientEventBuffer:
    ...
    async def text(self, delta, *, item_id=None):
        ...
        if len(self.pending) >= 256 or time.monotonic() - self.last_flush >= .2:
            await self.flush()

    async def flush(self):
        while self.pending:
            text, self.pending = self.pending[:2048], self.pending[2048:]
            await self.context.control.event("assistant_text_delta", "Original client response draft.",
                {"text": text, "request_id": self.request_id, "item_id": self.item_id,
                 "delta": True, **({"reset": True} if self.reset else {})})
            self.reset = False
            self.has_text = True
            self.events += 1
            self.total += len(text)
    ...
    async def summary(self, delta, *, item_id=None, summary_index=0):
        ...
        if len(self.summary_pending[key]) >= 256 or time.monotonic() - self.last_flush >= .2:
            await self.context.control.event("assistant_reasoning_summary", "Original client supplied a reasoning summary.",
                {"text": self.summary_pending.pop(key), "request_id": self.request_id,
                 "item_id": item_id, "summary_index": summary_index, "delta": True})
            self.events += 1
            self.last_flush = time.monotonic()
    ...
    async def finish(self, *, complete=True):
        ...
        for (item_id, summary_index), text in self.summaries.items():
            await self.context.control.event("assistant_reasoning_summary", "Original client supplied a reasoning summary.",
                {"text": text, "request_id": self.request_id, "item_id": item_id,
                 "summary_index": summary_index, "complete": complete, "delta": False})
            self.events += 1
        for item_id, text in self.drafts.items():
            await self.context.control.event("runtime_public_section", "Original client response draft.",
                {"text": text, "request_id": self.request_id, "item_id": item_id,
                 "complete": complete, "delta": False, "category": "draft"})
        self.finished = True

    async def request_activity(self, phase, *, payload=None):
        await self.context.control.event("runtime_request_activity", "Content supplied by Quantix",
