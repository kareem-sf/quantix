...
def grok_command(connection):
    if connection.get("settings", {}).get("executable_path"):
        raise RuntimeUnavailable(
            "Use the Grok Build component installed by Quantix for this connection."
        )
    ...
    if (
        os.environ.get("QUANTIX_GROK_VERSION") != GROK_VERSION
        or not configured
        or not Path(configured).is_absolute()
        or Path(configured).resolve() != expected
        or not expected.is_file()
        or expected.is_symlink()
    ):
        raise RuntimeUnavailable(
            "Prepare the pinned Grok Build component before connecting this account."
        )
    ...
    try:
        ...
        raise RuntimeUnavailable(
            "The installed Grok Build publisher manifest is unavailable. Prepare its component again."
        ) from None
    if metadata.get("version") != GROK_VERSION or not str(metadata.get("name", "")).startswith(
        "@xai-official/grok-"
    ):
        raise RuntimeUnavailable(
            "The Grok Build package does not match the version supported by this adapter."
        )
    return [str(expected)]
...
def _require_owned_configuration(home):
    ...
    if sys.platform == "darwin":
        paths.extend(
            [
                Path("/Library/Managed Preferences/ai.x.grok.plist"),
                Path("/Library/Preferences/ai.x.grok.plist"),
            ]
        )
    if os.name != "nt" and any(path.exists() for path in paths):
        raise RuntimeUnavailable(
            "Grok Build has centrally managed configuration. An administrator must provide an approved Tender-tool integration."
        )
    root = grok_home(home)
    for path in (home, root):
        if path.is_symlink():
            raise RuntimeUnavailable(
                "The Grok account directory must be a private local directory."
            )
    ...
    for relative in (
        ...
        if path.exists() or path.is_symlink():
            raise RuntimeUnavailable(
                "This Grok account has additional local configuration. Use a separate Quantix connection for Tender work."
            )
    for name in ("hooks", "plugins", "skills", "agents"):
        path = root / name
        if path.is_symlink() or (path.exists() and (not path.is_dir() or any(path.iterdir()))):
            raise RuntimeUnavailable(
                "This Grok account has additional hooks or extensions. Use a separate Quantix connection."
            )
    hook_paths = root / "hooks-paths"
    if hook_paths.is_symlink() or (
        hook_paths.exists()
        and (
            not hook_paths.is_file()
            or hook_paths.stat().st_size > 1024
            or hook_paths.read_text(encoding="utf-8").strip()
        )
    ):
        raise RuntimeUnavailable(
            "This Grok account has additional hook locations. Use a separate Quantix connection."
        )
        ...
        while not acquired:
            try:
                ...
                if asyncio.get_running_loop().time() >= deadline:
                    raise RuntimeUnavailable(
                        "This Grok account is still busy. Retry after its current work or sign-in finishes."
                    ) from None
...
def configure(home, *, model=None, output_limit=1024, bridge=None):
    ...
    lines = [
        "# Owned by Quantix. OAuth credentials remain in the official client's separate auth store.",
        "[cli]",
        "auto_update = false",
        "use_leader = false",
        "[auth]",
        ...
        "[features]",
        *[f"{name} = false" for name in features],
        "[relay]",
        "enabled = false",
        "[memory]",
        "enabled = false",
        ...
        "paths = []",
        "[permission]",
        'deny = ["Bash", "Read(**)", "Write", "Edit", "Grep", "WebFetch"]'
        if bridge
        else 'deny = ["Bash", "Read", "Write", "Edit", "Grep", "WebFetch"]',
    ...
    if model:
        ...
        lines.extend(
            [
                ...
                f"max_completion_tokens = {output_limit}",
                "max_retries = 0",
            ]
        )
    if bridge:
        lines.extend(
            [
                "[mcp_servers.quantix]",
                f"url = {json.dumps(bridge.url)}",
                'bearer_token_env_var = "QUANTIX_MCP_TOKEN"',
                "enabled = true",
...
def environment(home, *, allow_browser=False, bridge=None):
    ...
    if allow_browser:
        ...
        env = sign_in_browser_environment(
            env, account_key="GROK_HOME", account_home=grok_home(home)
        )
        for name in (
            "DISPLAY",
            "WAYLAND_DISPLAY",
...
async def spawn(command, work, env, *, limit=1024 * 1024):
    ...
    if not host or not Path(host).is_absolute() or not Path(host).is_file():
        raise RuntimeUnavailable(
            "The native Quantix process owner is unavailable. Prepare the AI component again."
        )
    ...
    if process.returncode is None:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeUnavailable(
                    "Grok Build did not close within its shutdown allowance. Its result was withheld."
                ) from None
