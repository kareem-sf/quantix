...
def codex_config(home, *, work=None, bridge=None):
    ...
    overrides = {
        ...
        "features.code_mode.enabled": False, "features.skill_mcp_dependency_install": False,
    ...
    return CodexConfig(
        cwd=str(work or home), env=env, experimental_api=False,
        client_name="quantix", client_title="Quantix Tender Office",
...
def turn_failure(terminal, model: str) -> str:
    ...
    if reported and info in {"usageLimitExceeded", "contextWindowExceeded", "sessionBudgetExceeded"}:
...
async def execute_codex(route, connection, credentials, context, instruction, output_type,
    ...
    if connection.get("billing") != "subscription":
        raise RuntimeUnavailable("This Codex client does not expose a hard inference-request cap. Use an OpenAI API connection for metered work with a strict budget.")
    if connection["auth_type"] != "client_login" or credentials:
        raise RuntimeUnavailable("Use managed ChatGPT sign-in for this Codex connection. API-key work belongs to the separately budgeted OpenAI API connection.")
    if route.get("web_search"):
        raise RuntimeUnavailable("This Codex connection exposes Tender tools only. Select an approved API research connection for web research.")
    ...
    discovery = (
        " The quantix MCP server provides these callable tools: "
        + ", ".join(bridge.names) + ". "
    ...
    try:
        async with bridge.serve():
            async with asyncio.timeout(timeout):
                async with owned_client(AsyncCodex(codex_config(home, work=work, bridge=bridge))) as client:
                    account = await client.account()
                    account_value = account.model_dump(mode="json", by_alias=True).get("account")
                    if not account_value:
                        raise RuntimeUnavailable("Sign in to the Quantix Codex connection before starting Tender work.")
                    if account_value.get("type") != "chatgpt":
                        raise RuntimeUnavailable("This Codex client is not using managed ChatGPT sign-in. Select the account's actual billing route before starting work.")
                    thread_settings = dict(
                        model=route["model_id"], cwd=str(work),
                        approval_mode=ApprovalMode.deny_all, sandbox=Sandbox.read_only,
                        base_instructions=instruction + boundary,
                    )
                    if binding and binding.get("provider_session_id"):
                        thread = await client.thread_resume(binding["provider_session_id"], **thread_settings)
                    else:
                        thread = await client.thread_start(ephemeral=binding is None, **thread_settings)
                    usage["session_id"] = thread.id
                    if binding:
                        await context.control.event("runtime_session", "Original client session started.",
                    ...
                    if not checking:
                        await events.request_activity("prepared", payload={"instructions": instruction + boundary,
                            ...
                            "settings": {"model": route["model_id"], "effort": route.get("reasoning"), "summary": "auto",
                                "approval_mode": "deny_all", "sandbox": "read_only"}, "billing": connection.get("billing")})
                    ...
                    try:
                        async for notification in turn.stream():
                            ...
                            if notification.method == "model/rerouted":
                                ...
                                await context.control.event("runtime_model_reported",
                                    "Codex reported its selected model.",
                                    {"connection_id": connection["id"], "requested_model": route["model_id"], "actual_model": actual})
                                if actual != route["model_id"]:
                                    raise RuntimeUnavailable("Codex reported a model reroute that was not approved for this request. The result was withheld.")
                            elif isinstance(payload, ThreadTokenUsageUpdatedNotification):
                                record_token_usage(usage, payload.token_usage, resuming=bool(binding and binding.get("provider_session_id")))
                            elif notification.method == "item/agentMessage/delta" and not checking:
                                await events.text(payload.delta, item_id=payload.item_id)
                            elif notification.method == "item/reasoning/summaryTextDelta" and not checking:
                                await events.summary(payload.delta, item_id=payload.item_id, summary_index=payload.summary_index)
                            elif notification.method == "item/reasoning/summaryPartAdded" and not checking:
                                ...
                                if item.type == "reasoning" and notification.method == "item/completed" and not checking:
                                    for index, text in enumerate(getattr(item, "summary", None) or []):
                                        events.summary_snapshot(text, item_id=item.id, summary_index=index)
                                if item.type in {"commandExecution", "fileChange", "collabAgentToolCall", "webSearch"}:
                                    raise RuntimeUnavailable("Codex attempted a native action outside the approved Quantix tool boundary. Use the reviewed Quantix tools for this work.")
                                if item.type == "mcpToolCall":
                                    ...
                                    if not checking:
                                        await context.control.event("runtime_tool_activity", "Original client tool activity.",
                                            {"tool": item.tool, "phase": "started" if notification.method == "item/started" else "completed",
                                             "provider_call_id": getattr(item, "id", None), "request_id": events.request_id})
                                        if getattr(item, "error", None) is not None:
                                            await events.request_activity("observed", payload={"provider_tool_error": {
                                                "tool": item.tool, "provider_call_id": item.id, "error": item.error.message}})
                            elif isinstance(payload, TurnCompletedNotification):
                                terminal = payload.turn
                    except BaseException as error:
                        activity_cancelled = isinstance(error, (asyncio.CancelledError, InterruptedError))
                        bridge.closed = True
                        try:
                    ...
                    if terminal is None:
                        raise RuntimeConnectionFailure("The Codex stream ended without a final turn status.")
                    ...
                    if terminal.status == TurnStatus.failed:
                        info = terminal.error.codex_error_info.model_dump(mode="json", by_alias=True) if terminal.error and terminal.error.codex_error_info else None
                        retryable = info in {"serverOverloaded", "internalServerError"} if isinstance(info, str) else False
                        if isinstance(info, dict):
                            for name in ("httpConnectionFailed", "responseStreamConnectionFailed", "responseStreamDisconnected", "responseTooManyFailedAttempts"):
                                if name in info:
                                    status = info[name].get("httpStatusCode")
                                    retryable = status is None or transient_status(status)
                        if retryable:
                            raise RuntimeConnectionFailure("The Codex service connection failed temporarily.")
                    if terminal.status != TurnStatus.completed:
                        raise RuntimeUnavailable(turn_failure(terminal, route["model_id"]))
                    if not checking:
                        await events.finish()
                        await events.request_activity("completed", payload={"usage": usage, "output": final_message(terminal), "provider_turn_id": terminal.id})
                    completed = True
                    return {"output": await bridge.result(final_message(terminal)),
                            "usage": usage, "web_sources": []}
    finally:
        try:
            if activity_prepared and not completed:
                await events.finish(complete=False)
                await events.request_activity("cancelled" if activity_cancelled else "failed", payload={"usage": usage, "detail": "The original-client request ended before successful completion."})
        finally:
