...
def create_router(repo):
    ...
    @router.post(
        "/ai/connections/{connection_id}/generation-preview", response_model=GenerationPreview
    )
    def preview(connection_id: str, request: GenerationPreviewRequest):
        with connections.authority_guard(), repo.atomic():
            ...
            model = next(
                (
                    item
                    for item in connections.models(connection_id)
                    if item["model_id"] == request.model_id
                ),
                None,
            )
            if model is None:
            ...
            return generation_preview(
                {**connection, "_model": model}, request.settings.model_dump()
            )
