...
def project_tools():
    ...
    async def inspect_project_map(ctx: RunContextWrapper[OfficeContext], offset: int, limit: int) -> str:
        ...
        return json.dumps(_clean({
            "nodes": view["nodes"][offset:offset + limit], "total_nodes": len(view["nodes"]),
            "review_scopes": view["review_scopes"][offset:offset + limit], "total_review_scopes": len(view["review_scopes"]),
            "coverage": view["coverage"], "directory_areas": view["directory_areas"],
            "next_offset": offset + limit if max(len(view["nodes"]), len(view["review_scopes"])) > offset + limit else None,
        }), ensure_ascii=False)

    @function_tool(failure_error_function=None)
    async def inspect_submission_requirements(ctx: RunContextWrapper[OfficeContext], offset: int, limit: int) -> str:
        ...
        return json.dumps(_clean({"requirements": rows, "next_offset": offset + limit if len(rows) == limit else None}), ensure_ascii=False)

    @function_tool(failure_error_function=None)
    async def inspect_generated_documents(ctx: RunContextWrapper[OfficeContext], offset: int, limit: int) -> str:
        ...
        return json.dumps(_clean({"outputs": rows[offset:offset + limit], "total": len(rows), "next_offset": offset + limit if offset + limit < len(rows) else None}), ensure_ascii=False)
