...
def project_tools():
    ...
    async def inspect_project_map(ctx: ToolContext[OfficeContext], offset: int, limit: int) -> str:
        ...
        result = {
            ...
            "total_review_scopes": len(reviews),
            "coverage": view["coverage"],
            "directory_areas": view["directory_areas"],
            "next_offset": offset + limit
            if max(len(nodes), len(reviews)) > offset + limit
            else None,
        }
        return json.dumps(_clean(result), ensure_ascii=False)

    @scoped_tool
    async def inspect_submission_requirements(
    ...
    @scoped_tool
    async def inspect_generated_documents(
        ctx: ToolContext[OfficeContext], offset: int, limit: int
    ) -> str:
...
def publish_project(output, context):
    ...
    if output.project_map_nodes:
        ...
        nodes = [
            service.propose(
                context.tender_id, node.model_dump(), origin="agent", run_id=context.run_id
            )
            for node in output.project_map_nodes
        ]
    if output.submission_requirements:
        ...
        requirements = [
            ...
            for requirement in output.submission_requirements
        ]
    return {
        "project_map_nodes": nodes,
        "submission_requirements": requirements,
        "programme_proposal": output.programme_proposal.model_dump(mode="json")
