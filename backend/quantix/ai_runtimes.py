...
class RuntimeService:
    ...
    async def status(self, connection_id):
        ...
        if state["state"] == "ready":
            ...
            state.update(
                {
                    key: account.get(key)
                    for key in ("state", "detail", "login_url", "user_code", "docs_url")
                    if key in account
                }
            )
        return state
    ...
    async def logout(self, connection_id):
        ...
        if connection["protocol"] not in {"codex", "grok_build"}:
            raise ValueError(
                "Direct API connections use an API key instead of original-client sign-out."
            )
    ...
    *,
    ...
    if not is_subscription_profile(connection):
        raise RuntimeUnavailable(
            "This AI connection does not use an approved original-client subscription route."
        )
    from .ai_worker_client import AIWorkerClient
