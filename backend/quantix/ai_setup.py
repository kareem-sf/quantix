...
class AISetupService:
    ...
    def _software(self, connection):
        if is_direct_profile(connection):
            ...
            return SetupSoftware.model_validate(direct_runtime_status(connection)).model_dump(
                mode="json"
            )
        if is_subscription_profile(connection):
            return SetupSoftware.model_validate(self.components.status(connection)).model_dump(
                mode="json"
            )
        return {
        ...
        with self.repo.db.connect() as conn:
            if conn.execute(
                ...
                for row in conn.execute("SELECT data_json FROM tender_ai_policy"):
                    ...
                    for route in [
                        policy.get("manager"),
                        policy.get("specialist"),
                        *policy.get("role_routes", {}).values(),
                    ]:
    ...
    def get(self, identifier):
        ...
        detail = saved.get(
            "detail", "Connect this AI account to prepare its software and check access."
        )
        if (
            is_subscription_profile(connection)
            and not saved.get("stage")
            and software["state"] == "ready"
        ):
            stage, detail = (
                "needs_sign_in",
                "The original client is prepared. Refresh the account or sign in through its official flow.",
            )
        if not method["available"]:
            stage, detail = "attention", method["unavailable_reason"] or method["detail"]
        if check["status"] == "passed" and (
            saved.get("checked_revision") != connection["revision"]
            or saved.get("checked_component_version") != software["version"]
        ):
            check = dict(_CHECK_EMPTY)
            stage, detail = (
                "ready_to_check",
                "This account changed. Check it again before using the new settings.",
            )
        if selected and not any(model["model_id"] == selected for model in models):
            stage, detail = (
                "choose_model",
                "The previously selected AI is unavailable. Choose another model; nothing has been switched automatically.",
            )
        if (
            is_direct_profile(connection)
            and connection["credential_state"] == "missing"
            and not self._busy(identifier)
        ):
            check = dict(_CHECK_EMPTY) | {"reserved_usd": self.store.unresolved_cost(identifier)}
            stage, detail = (
                "needs_credentials",
                "This account's access key is no longer available. Enter it again, or repair its access settings in More options.",
            )
        ...
        if not connection["enabled"]:
            stage, detail = (
                "attention",
                "This AI account is turned off. Enable it in More options before continuing.",
            )
        if not supported:
            stage, detail = (
                "attention",
                method["unavailable_reason"]
                or "This saved AI account is retired. Choose one of the five supported provider routes.",
            )
            check = dict(_CHECK_EMPTY) | {"reserved_usd": self.store.unresolved_cost(identifier)}
        check["reserved_usd"] = self.store.unresolved_cost(identifier)
        return SetupAccount(
            ...
            supported=supported,
            access_kind=method.get("access_kind", "api_key"),
            stage=stage,
            detail=detail,
            ...
            recommended_model_id=recommendation_id,
            recommendation=recommendation,
            login_url=saved.get("login_url"),
            user_code=saved.get("user_code"),
            check=check,
            subscription_usage=saved.get("subscription_usage")
    ...
    async def start(self, values):
        ...
        if is_direct_profile(connection):
            ...
            self._state(
                ...
                detail="Enter the access key from your AI account to continue."
                if missing
                else "Finding the AI models available to your account.",
                check=dict(_CHECK_EMPTY),
            )
            if not missing:
            ...
            self._state(
                ...
                detail="Sign in through the official client to continue."
                if software["state"] == "ready"
                else software["detail"],
                check=dict(_CHECK_EMPTY),
            )
        else:
            raise ValueError(
    ...
    def _launch(self, identifier, action, **values):
        ...
        if self._busy(identifier):
            raise ValueError(
                "This account is already being prepared. Wait or cancel its current action."
            )
        if action == "check":
            with (
                self.connections.lease(identifier, exclusive=True),
                self.connections.authority_guard(),
                self.repo.atomic(),
            ):
        ...
        task = asyncio.create_task(
            self._perform(identifier, action, **values), name=f"quantix-ai-setup-{identifier}"
        )
        self.tasks[identifier] = task
        task.add_done_callback(
            lambda finished: (
                self.tasks.pop(identifier, None) if self.tasks.get(identifier) is finished else None
            )
        )
    ...
    async def action(self, identifier, values):
        ...
        if request.sign_in_method is not None and not subscription:
            raise ValueError(
                "Direct API connections use an API key instead of original-client sign-in."
            )
        if direct and request.action in {"sign_in", "sign_out", "remove_software"}:
            raise ValueError(
                "This API connection uses bundled software. Enter its key or refresh its models in Settings."
            )
        if request.action == "set_subscription_extras":
            if not subscription or connection["protocol"] != "grok_build":
                raise ValueError(
                    "Provider-managed extras are available only for the Grok subscription connection."
                )
            if request.allow_provider_managed_extras is None:
                raise ValueError(
                    "Choose whether this Grok account may use provider-managed extras."
                )
            if self._busy(identifier):
                raise ValueError(
                    "Wait for setup to finish before changing this spending preference."
                )
                ...
                updated = current | {
                    "settings": {
                        **current.get("settings", {}),
                        "allow_provider_managed_extras": request.allow_provider_managed_extras,
                    }
                }
                for field in (
                    "credential_state",
                    ...
                    "session_only",
                    "_model",
                    "_checked_component_version",
                ):
                    updated.pop(field, None)
                updated = self.connections.update(identifier, updated)
                self.store.carry_spending_preference_checks(
                    identifier, old_revision, updated["revision"]
                )
            self._state(
                identifier,
                detail="Grok spending preference saved. Review the account's Tender approval before using it.",
            )
            return self.get(identifier)
        if request.action == "refresh_usage" and (
            not subscription or connection["protocol"] != "grok_build"
        ):
            raise ValueError(
                "Usage refresh is available only for the Grok subscription connection."
            )
        if subscription and request.action == "refresh_usage":
            if self._busy(identifier):
                raise ValueError(
                    "Wait for the current account action to finish before refreshing usage."
                )
            snapshot = await self._refresh_subscription_usage(identifier, force=True)
            detail = (
                (snapshot or {}).get("detail")
                or "Grok usage was refreshed. Review the current allowance, then check the selected model."
            )
        ...
        if request.action == "cancel":
            ...
            self._state(
                ...
                stage="cancelled",
                active=False,
                detail="Setup stopped. Your account and Tender records are preserved.",
                login_url=None,
                user_code=None,
            )
            ...
            if direct and connection["credential_state"] == "missing":
                ...
                self._launch(
                    identifier,
                    "refresh" if direct else request.action,
                    sign_in_method=request.sign_in_method,
                )
        elif request.action == "check":
            preview = (
                await self.check_preview_async(identifier)
                if subscription
                else self.check_preview(identifier)
            )
            if not preview["allowed"]:
                raise ValueError(preview["detail"])
            if request.check_fingerprint != preview["fingerprint"]:
                raise ValueError(
                    "The account or check cost changed. Review the displayed check again."
                )
            if (
                connection["billing"] in {"metered", "unknown"}
                and request.maximum_cost_usd != preview["maximum_cost_usd"]
            ):
                raise ValueError(
                    "Review and accept the displayed cost allowance before starting this check."
                )
            if preview["requires_unknown_cost_consent"] and not request.accept_unknown_cost:
                raise ValueError(
                    "Review the unknown-cost notice and explicitly accept this short connection check."
                )
            self._launch(
                identifier,
                "check",
                preview=preview,
                accept_unknown_cost=request.accept_unknown_cost,
            )
        elif request.action == "select_model":
            if self._busy(identifier):
                raise ValueError(
                    "Wait or cancel the current setup action before choosing another AI."
                )
            if not any(
                m["model_id"] == request.model_id for m in self.connections.models(identifier)
            ):
            ...
            valid = (
                ...
                and evidence.get("checked_revision") == connection["revision"]
                and evidence.get("checked_component_version") == current_software["version"]
                and current_software["state"] == "ready"
                and connection["credential_state"] != "missing"
            )
            self._state(
                identifier,
                selected_model_id=request.model_id,
                stage="ready" if valid else "ready_to_check",
                check=evidence["check"] if valid else dict(_CHECK_EMPTY),
                checked_revision=evidence.get("checked_revision") if valid else None,
                checked_component_version=evidence.get("checked_component_version")
                if valid
                else None,
                detail="Your checked model is selected."
                if valid
                else "Your choice is saved. Check this model before using it.",
            )
    ...
    async def _perform(self, identifier, action, **values):
        try:
            ...
            if not is_supported_profile(connection):
                ...
                if not method["available"]:
                    self._state(
                        identifier,
                        stage="attention",
                        detail=method["unavailable_reason"] or method["detail"],
                        active=False,
                    )
                    return
            if action == "check":
                await self._check(
                    identifier,
                    values["preview"],
                    accept_unknown_cost=values.get("accept_unknown_cost", False),
                )
                return
            if is_direct_profile(connection):
                if connection["credential_state"] == "missing":
            ...
            self._state(
                identifier,
                stage="cancelled",
                detail="Setup was stopped. Choose Retry when you are ready.",
                login_url=None,
                user_code=None,
            )
            raise
            ...
            reference = record_exception(
                "ai_setup_failed", error, phase=action, connection_id=identifier
            )
            known = isinstance(
                error, (RuntimeUnavailable, RuntimeConnectionFailure, ValueError)
            ) or type(error).__name__ in {"DirectProviderError", "DirectProviderUnavailable"}
            message = (
                str(error)
                if known
                else "This AI could not finish setup. Check your account details and choose Retry."
            )
            ...
            self._state(
                identifier, stage="attention", detail=message, login_url=None, user_code=None
            )
        finally:
            if (
                is_subscription_profile(self.connections.get(identifier))
    ...
    async def _perform_subscription(self, identifier, action, *, sign_in_method=None, **_values):
        ...
        if action in {"prepare", "repair"}:
            ...
            with self.connections.lease(identifier, exclusive=True) as current:
                result = await self.components.prepare(
                    current,
                    repair=action == "repair",
                    progress=lambda value: self._state(
                        identifier,
                        progress=value.get("progress"),
                        detail=value.get("detail", "Preparing selected AI software."),
                    ),
                )
            with self.connections.lease(identifier, exclusive=True) as current:
            ...
            if status.get("state") == "signed_in":
                self._state(
                    identifier,
                    stage="discovering",
                    detail="The original client confirmed its saved sign-in. Finding available models.",
                ...
                self._state(
                    ...
                    detail=status.get(
                        "detail",
                        "The original client is ready. Sign in through its official account flow.",
                    ),
                    progress=result.get("progress"),
                ...
                self._state(
                    ...
                    detail=status.get(
                        "detail",
                        "The original client is prepared but could not confirm its saved sign-in.",
                    ),
                    progress=result.get("progress"),
                )
            return
        if action == "remove_software":
            ...
            self._state(
                identifier,
                stage="needs_preparation",
                detail=result.get("detail", "Prepare the original client before continuing."),
        ...
        if action == "sign_in":
            ...
            with self.connections.lease(identifier, exclusive=True):
                ...
                if state == "login_pending":
                    self._state(
                        ...
                        stage="needs_sign_in",
                        active=True,
                        detail=result.get("detail", "Complete the official sign-in to continue."),
                    ...
                    while True:
                        ...
                        if terminal.get("state") == "login_pending":
                            self._state(
                                ...
                                active=True,
                                detail=terminal.get(
                                    "detail", "Complete the official sign-in to continue."
                                ),
                                login_url=terminal.get("login_url"),
                                user_code=terminal.get("user_code"),
                            )
                            continue
                        if terminal.get("state") != "login_pending":
                            break
            ...
            if state == "signed_in":
                self._state(
                    identifier,
                    stage="discovering",
                    detail="The original client confirmed sign-in. Finding the models available to this account.",
                    login_url=None,
                    user_code=None,
                )
                await self._discover(identifier)
                ...
                self._state(
                    identifier,
                    stage="attention",
                    detail=result.get("detail", "The original client did not confirm sign-in."),
                    login_url=result.get("login_url"),
                    user_code=result.get("user_code"),
                )
            return
        if action == "sign_out":
            ...
            if state == "signed_out":
                self._state(
                    ...
                    detail=result.get(
                        "detail",
                        "The original client signed out. Sign in again before checking access.",
                    ),
                    login_url=None,
                    user_code=None,
                )
                ...
                self._state(
                    ...
                    detail=result.get(
                        "detail",
                        "The original client did not confirm sign-out. Review the original account controls and retry.",
                    ),
                    login_url=None,
                    user_code=None,
                )
        ...
        if action == "refresh":
            ...
            if state == "signed_in":
                ...
                self._state(
                    identifier,
                    stage="needs_sign_in",
                    detail=status.get("detail", "Sign in through the official client to continue."),
                    login_url=status.get("login_url"),
                    user_code=status.get("user_code"),
                )
            else:
                self._state(
                    ...
                    stage="attention",
                    detail=status.get(
                        "detail", "The original client could not confirm its saved sign-in."
                    ),
                    login_url=status.get("login_url"),
                    user_code=status.get("user_code"),
                )
    ...
    async def _discover(self, identifier):
        previous = self.store.get(identifier).get("selected_model_id") or self._existing_model(
            identifier
        )
        self._state(
            identifier,
            stage="discovering",
            detail="Finding the AI models available to your account…",
            login_url=None,
            user_code=None,
        )
        ...
        for value in values:
            ...
            if (
                proof.get("check", {}).get("status") == "passed"
                and proof.get("checked_revision") == revision
                and proof.get("checked_component_version") == software["version"]
            ):
        ...
        checked = (
            exists
            and proof.get("check", {}).get("status") == "passed"
            and proof.get("checked_revision") == self.connections.get(identifier)["revision"]
            and proof.get("checked_component_version") == software["version"]
            and software["state"] == "ready"
        )
        if checked:
            self._state(
                ...
                stage="ready",
                check=proof["check"],
                checked_revision=proof["checked_revision"],
                checked_component_version=proof["checked_component_version"],
                detail="Account access and available models refreshed. Your checked model is still ready.",
            )
            return
        self._state(
            identifier,
            selected_model_id=selected,
            stage="ready_to_check" if exists else "choose_model",
            check=dict(_CHECK_EMPTY),
            checked_revision=None,
            detail="A model is selected. Review it and check the connection."
            if exists
            else "Choose an available model. Quantix has not changed your previous choice.",
        )
    ...
    async def _refresh_subscription_usage(self, identifier, *, force=False, guarded=False):
        ...
        if guarded:
            snapshot = (
                await self.worker.subscription_usage(connection)
                if self.worker is not None
                else unknown_subscription()
            )
        else:
            with self.connections.lease(identifier, exclusive=True) as current:
                snapshot = (
                    await self.worker.subscription_usage(current)
                    if self.worker is not None
                    else unknown_subscription()
                )
        return snapshot
    ...
    def _preview(self, identifier, *, usage=None):
        ...
        model = next(
            (m for m in account["models"] if m["model_id"] == account["selected_model_id"]), None
        )
        subscription = is_subscription_profile(connection)
        allowed = (
            account["supported"]
            and account["stage"] in {"ready_to_check", "ready", "attention", "cancelled"}
            and account["software"]["state"] == "ready"
        ...
        if subscription:
            ...
            if not prices:
                ...
                amount = (
                    (
                        Decimal(16384) * Decimal(str(prices["input_per_million"]))
                        + Decimal(1024) * Decimal(str(prices["output_per_million"]))
                    )
                    * requests
                    / Decimal(1000000)
                )
        ...
        if (
            subscription
            and not allowed
            and account["stage"] not in {"ready_to_check", "ready", "attention", "cancelled"}
        ):
        ...
        body = {
            ...
            "component_version": account["software"]["version"],
            "max_requests": requests,
            "max_output_tokens": max_output,
            "max_input_tokens": max_input,
            "requires_unknown_cost_consent": requires_unknown,
            "subscription_check": subscription,
            "subscription_usage": self._subscription_usage_authority(usage)
            if subscription and connection["protocol"] == "grok_build"
            else None,
        }
        return SetupCheckPreview(
            allowed=allowed,
            detail=detail,
            model_id=account["selected_model_id"],
            maximum_cost_usd=cost,
    ...
    def check_preview(self, identifier):
        connection = self.connections.get(identifier)
        usage = (
            self._subscription_snapshot_for(identifier)
            if is_subscription_profile(connection)
            else None
        )
    ...
    async def check_preview_async(self, identifier):
        ...
        if (
            is_subscription_profile(connection)
            and connection["protocol"] == "grok_build"
            and not self._busy(identifier)
        ):
    ...
    async def _check(self, identifier, preview, *, accept_unknown_cost=False):
        ...
        with self.connections.lease(identifier, exclusive=True) as connection:
            ...
            if subscription and connection["protocol"] == "grok_build":
                ...
                if not included_only_current(usage):
                    raise ValueError(
                        "Grok's current included allowance could not be confirmed. Refresh usage in the official settings, then try the check again."
                    )
            models = self.connections.models(identifier)
            ...
            body = {
                ...
                "component_version": self._software(connection)["version"],
                "max_requests": preview["max_requests"],
                "max_output_tokens": preview["max_output_tokens"],
                "max_input_tokens": preview["max_input_tokens"],
                "requires_unknown_cost_consent": preview["requires_unknown_cost_consent"],
                "subscription_check": subscription,
                "subscription_usage": self._subscription_usage_authority(usage)
                if subscription and connection["protocol"] == "grok_build"
                else None,
            ...
            if hashlib.sha256(dump(body).encode()).hexdigest() != preview["fingerprint"]:
                raise ValueError(
                    "The account changed before its check started. Review the check again."
                )
            route = {
            ...
            worker_requests = (
                ...
                if connection["protocol"] == "grok_build"
                else max(1, int(connection.get("settings", {}).get("max_turns", 12)))
            )
            selected = connection | {
                "_model": model,
                "_checked_component_version": body["component_version"],
                "_subscription_check": subscription,
                "_execution_limits": {
                    "max_requests": worker_requests,
                    "max_output_tokens": 1024,
                    "context_window": model["capabilities"].get("context_window"),
                },
            }
            if subscription:
                meter = SubscriptionCheckMeter(
                    self.repo,
                    connection,
                    model,
                    max_requests=GROK_CHECK_MAX_ROUNDS
                    if connection["protocol"] == "grok_build"
                    else None,
                )
            else:
                meter = SetupCheckMeter(
                    ...
                    unknown_cost_accepted=accept_unknown_cost
                    and preview["requires_unknown_cost_consent"],
                )
            self._state(
                identifier,
                check={
                    "status": "checking",
                    "detail": "Checking the selected AI with a generic instruction.",
                },
            )
            try:
                credentials = self.connections.credentials(identifier)
                ...
                if not result.get("tools_supported"):
                    raise ValueError(
                        "The AI did not call the connection-check tool exactly once. Retry the access check or choose another model."
                    )
                if not result.get("output_supported"):
                    raise ValueError(
                        "The AI did not return the unchanged connection-check value in structured output. Retry the access check or choose another model."
                    )
                ...
                self._state(
                    ...
                    check=meter.summary(
                        ...
                        "This check did not finish successfully. Any uncertain charge remains recorded.",
                    ),
                )
                raise
        ...
        with self.connections.authority_guard(), self.repo.atomic():
            current = self.connections.get(identifier)
            if (
                current["revision"] != checked_revision
                or self.store.get(identifier).get("selected_model_id") != model["model_id"]
            ):
                raise ValueError(
                    "The account changed after its check. The previous result cannot approve these new settings; check it again."
                )
            ...
            if (
                model["capabilities"].get("tools") is not True
                or model["capabilities"].get("structured_output") is not True
            ):
            ...
            evidence = {
                ...
                "checked_component_version": body["component_version"],
                "check": meter.summary(
                    "passed", "The selected AI responded and completed its connection check."
                ),
            }
            self.store.save_model_check(
                identifier, model["model_id"], evidence, previous_revision=original_revision
            )
            self._state(
                identifier,
                stage="ready",
                detail="This AI account is ready. Choose it when setting up a Tender.",
                **evidence,
            )
