...
class AIComponentService:
    ...
    def component_id(self, connection: dict) -> str:
        component = {"codex": "client-codex", "grok_build": "client-grok"}.get(
    ...
    def _component(self, connection):
        ...
        if not definition or target not in definition.get("locks", {}):
            raise ComponentUnavailable(
                "The selected AI software has no supported binary installation for this operating system and processor."
            )
    ...
    def _active(self, identifier, fingerprint, *, deep=False):
        ...
        if receipt.get("fingerprint") != fingerprint or not receipt_valid(
            path, deep=deep, allowed_roots=(python_root,)
        ):
            return None
        ...
        try:
            ...
            if previous.get("state") == "preparing":
                try:
                    ...
                    return self._status(
                        identifier,
                        "preparing",
                        previous.get("detail", "Preparing selected AI software."),
            ...
            if active:
                return self._status(
                    identifier,
                    "ready",
                    "The selected AI software is installed. Account access is checked separately.",
                    100,
                    active.name,
                )
            if previous.get("state") in {"attention", "preparing"}:
                return self._status(
                    ...
                    "attention",
                    previous.get("detail")
                    if previous.get("state") == "attention"
                    else "AI software preparation was interrupted. Select Prepare to continue.",
                )
            if (directory / "active.json").exists():
                return self._status(
                    identifier,
                    "attention",
                    "The selected AI software needs preparation or repair before it can be used.",
                )
            return self._status(
                identifier,
                "missing",
                "Prepare the software for this AI connection when you need it.",
            )
        except (ComponentUnavailable, OSError, KeyError, TypeError) as exc:
            detail = (
                str(exc)
                if isinstance(exc, ComponentUnavailable)
                else "Quantix could not read the selected AI software installation. Repair its software."
            )
        ...
        env.update(
            {
                ...
                "UV_PYTHON_BIN_DIR": str(self.root / "bin"),
    ...
    async def _run(self, command, *, cwd, env, seconds=1200):
        ...
        try:
            ...
            if code:
                raise ComponentUnavailable(
                    "AI software preparation could not finish. Check the network and available storage, then retry."
                )
        except BaseException as error:
            if not isinstance(error, asyncio.CancelledError):
                record_exception(
                    "ai_installer_process_failed",
                    error,
                    phase="installer",
                    duration_ms=int((asyncio.get_running_loop().time() - started) * 1000),
                )
    ...
    def _remove_directory(self, path):
        ...
        if resolved == self.root or not resolved.is_relative_to(self.root):
            raise ComponentUnavailable(
                "The AI software removal path is outside its private directory."
            )
        if resolved.exists():
    ...
    async def _archive(self, asset, destination, cancelled, *, allow_links=True):
        ...
        try:
            ...
            await _blocking(
                extract_archive, archive, destination, cancelled, allow_links, cancelled=cancelled
            )
        finally:
        ...
        async with _acquire(self._guard("shared-runtimes")):
            ...
            for kind in ("uv", "python"):
                ...
                directory = (
                    ...
                    else shared / f"{kind}-{asset['sha256'][:20]}"
                )
                if not await _blocking(_deep_receipt_valid, directory, cancelled=cancelled):
                    ...
                    try:
                        ...
                        if not executable.is_file():
                            raise ComponentUnavailable(
                                "The downloaded AI runtime is missing its executable."
                            )
                        files = await _blocking(
                            inventory, directory, cancelled, cancelled=cancelled
                        )
                        ...
                        if kind == "python":
                            ...
                            required += [
                                ...
                                for path in (directory / "python").glob("*.dll")
                            ]
                        if any(name not in files for name in required):
                            raise ComponentUnavailable(
                                "The private AI runtime is missing a required library."
                            )
                        _atomic_json(
    ...
    async def prepare(self, connection, *, repair=False, progress=None) -> dict:
        ...
        async with _acquire(self._guard(identifier)):
            active = (
                await _blocking(
                    lambda: self._active(identifier, fingerprint, deep=True), cancelled=cancelled
                )
                if not repair
                else None
            )
            ...
            directory = self._directory(identifier)
            candidate = (
                directory / "versions" / f"{definition['version']}-{fingerprint[:12]}-{uuid4().hex}"
            )
            ...
            try:
                ...
                if identifier == "client-grok":
                    ...
                    if (
                        metadata.get("name") != asset["package"]
                        or metadata.get("version") != definition["version"]
                        or metadata.get("os") != [asset["os"]]
                        or metadata.get("cpu") != [asset["cpu"]]
                    ):
                        raise ComponentUnavailable(
                            "The downloaded Grok package does not match the selected version and computer."
                        )
                    ...
                    if (
                        ...
                        or compressed.stat().st_size == 0
                        or executable.exists()
                    ):
                        raise ComponentUnavailable(
                            "The official Grok package is missing its expected compressed client."
                        )
                    await report(
                        85, "Unpacking the official Grok client in its private software folder."
                    )
                    ...
                    if not executable.is_file() or executable.stat().st_size == 0:
                        raise ComponentUnavailable(
                            "The official Grok client could not be unpacked. Repair its software."
                        )
                    if os.name != "nt":
                ...
                required += [
                    path.relative_to(candidate).as_posix() for path in worker.rglob("*.py")
                ]
                files = await _blocking(inventory, candidate, cancelled, cancelled=cancelled)
                if self._component(connection)[-1] != fingerprint:
                    raise ComponentUnavailable(
                        "Quantix's AI software files changed during preparation. Select Prepare again."
                    )
                _atomic_json(
                    candidate / "receipt.json",
                    {
                        ...
                        "python_root": base["python"][0].relative_to(self.root).as_posix(),
                        "files": files,
                        "required": required,
                ...
                _atomic_json(
                    ...
                    {"version": candidate.name, "fingerprint": fingerprint},
                )
                result = self._status(
                    identifier,
                    "ready",
                ...
                detail = (
                    "AI software preparation was interrupted. Select Prepare to continue."
                    if isinstance(exc, (asyncio.CancelledError, InterruptedError))
                    else (
                        str(exc)
                        if isinstance(exc, ComponentUnavailable)
                        else "AI software preparation could not finish. Check the network and available storage, then retry."
                    )
                )
                _atomic_json(
                    directory / "state.json", self._status(identifier, "attention", detail)
                )
                if not isinstance(exc, asyncio.CancelledError):
                    record_exception(
                        "ai_component_prepare_failed",
                        exc,
                        phase="preparation",
                        component_id=identifier,
                        component_version=definition.get("version"),
    ...
    def _required_files(self, identifier, directory, manifest, target):
        ...
        required += [
            site / package / "__init__.py"
            for package in ("mcp", "pydantic", "pydantic_core", "httpx2", "jsonschema")
        ]
        if identifier == "client-codex":
            ...
            binaries = list(
                (site / "codex_cli_bin").rglob("codex.exe" if os.name == "nt" else "codex")
            )
            if not any(path.is_file() for path in binaries):
                raise ComponentUnavailable(
                    "The selected Codex wheel is missing its original client binary."
                )
            ...
            required += [
                inside(client, native["assets"][target]["executable"]),
                client / "package.json",
                client / "THIRD_PARTY_NOTICES.md",
            ]
        if not all(path.is_file() and path.stat().st_size for path in required):
            raise ComponentUnavailable(
                "The selected AI software is missing a required installed file. Repair its software."
            )
    ...
    def worker_command(self, connection) -> list[str]:
        ...
        if directory is None:
            ...
            raise ComponentUnavailable(
                "Prepare or repair the selected AI software before using this connection."
            )
        python = directory / "venv" / ("Scripts/python.exe" if os.name == "nt" else "bin/python")
        return [
            str(native_host()),
            ...
            "-I",
            "-B",
            str(directory / "worker" / "worker_entry.py"),
        ]
    ...
    def environment(self, connection, command=None) -> dict[str, str]:
        ...
        if not directory.is_relative_to(
            self._directory(self.component_id(connection)) / "versions"
        ):
            raise ComponentUnavailable(
                "The AI worker command does not belong to this connection's selected software."
            )
        ...
        if self.component_id(connection) == "client-grok":
            ...
            if not isinstance(executable, str) or not isinstance(version, str):
                raise ComponentUnavailable(
                    "The Grok software receipt is missing its client version. Repair its software."
                )
            values.update(
                {
                    "QUANTIX_GROK_BINARY": str(inside(directory, executable)),
                    "QUANTIX_GROK_VERSION": version,
                }
            )
        return values
    ...
    def lease(self, connection):
        ...
        try:
            ...
            raise ComponentUnavailable(
                "The selected AI software is being prepared or removed. Wait for it to finish."
            ) from None
        try:
            yield command
        finally:
        ...
        try:
            with self._guard(identifier).acquire(timeout=0):
                ...
                if any(self._leased(identifier, path.name) for path in versions):
                    raise ComponentUnavailable(
                        "This AI software is still in use. Stop its active work before removing it."
                    )
                ...
                value = self._status(
                    identifier,
                    "missing",
                    "The shared AI software was removed. Private connection sign-in files were kept.",
                )
                _atomic_json(directory / "state.json", value)
                return value
        except Timeout:
            raise ComponentUnavailable(
                "This AI software is being prepared. Cancel preparation before removing it."
            ) from None
