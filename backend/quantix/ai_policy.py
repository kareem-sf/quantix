...
def search_accounting(data: dict) -> dict[str, bool | int]:
    ...
    if data.get("search_accounting_version") != _SEARCH_ACCOUNTING_VERSION or any(
        key not in data for key in required[1:]
    ):
        return {
            "calls": 0,
            "unknown": True,
...
def route_request_cost(
    ...
    if not prices:
        raise ValueError(
            "This paid AI needs confirmed prices and a Tender spending allowance. Open AI setup to finish these steps."
        )
    if route.get("web_search") and prices.get("web_search_per_call") is None:
        raise ValueError(
            "The price of online research is not confirmed for this AI. Choose another supported AI or review its pricing details."
        )
    ...
    if route.get("web_search"):
        amount += (
            Decimal(str(prices["web_search_per_call"]))
            * int(route.get("max_search_calls", 0))
            * count
        )
    return amount


def route_usage_cost(
    connection: dict,
    model: dict,
    route: dict,
    input_tokens: int,
...
class AIPolicyService:
    ...
    def _totals(self, conn, tender_id, run_id=None):
        ...
        for row in conn.execute(
            "SELECT data_json FROM ai_usage WHERE tender_id=? AND (? IS NULL OR run_id=?)",
            (tender_id, run_id, run_id),
        ):
    ...
    def get(self, tender_id):
        ...
        with self.repo.db.connect() as conn:
            row = conn.execute(
                "SELECT * FROM tender_ai_policy WHERE tender_id=?", (tender_id,)
            ).fetchone()
            spent, reserved, _ = self._totals(conn, tender_id)
            if row:
                saved = record(row)
                data = {k: v for k, v in saved["data"].items() if not k.startswith("_")} | {
                ...
                data = {
                    ...
                    "fallback_routes": [],
                    "run_budget_usd": None,
                    "tender_budget_usd": None,
                    "max_requests": 12,
                    "rationale": "",
                }
            needs_review = bool(
                restored and row and saved["data"].get("_restore_basis") != restored
            )
            return data | {
                "tender_id": tender_id,
    ...
    def validate_route(self, route, allowed):
        ...
        model = next(
            (
                m
                for m in self.connections.models(connection["id"])
                if m["model_id"] == route["model_id"]
            ),
            None,
        )
        if model is None:
        ...
        if capabilities.get("tools") is not True:
            raise ValueError(
                "The Tender Office requires model tool support. Confirm this model's documented capabilities in AI Connections."
            )
        ...
        if (
            ...
            and route["max_output_tokens"] > capabilities["max_output_tokens"]
        ):
    ...
    def update(self, tender_id, values):
        ...
        with self.connections.authority_guard(), self.repo.atomic() as conn:
            ...
            if any(
                ...
                and r["kind"] not in {"import", "index", "analysis"}
                for r in self.repo.list_runs(tender_id)
            ):
                raise ValueError(
                    "Stop the current tender work before changing its AI permissions or budget."
                )
            ...
            for route in [
                request.manager,
                request.specialist,
                *request.role_routes.values(),
                *request.fallback_routes,
            ]:
                if route:
                    self.validate_route(route, request.allowed_connection_ids)
                    account = self.connections.get(route.connection_id)
                    if (
                        allows_extras(account)
                        and request.provider_managed_extras.get(account["id"])
                        != account["revision"]
                    ):
                        raise ValueError(
                            "Review and approve Grok's provider-managed extras for this account before using it in the Tender. Quantix cannot guarantee a Tender spending cap for these extras."
                        )
            if any(
                identifier not in request.allowed_connection_ids
                or type(revision) is not int
                or revision < 1
                for identifier, revision in request.provider_managed_extras.items()
            ):
                raise ValueError(
                    "Extra-spending approval must identify an allowed AI account and its displayed revision."
                )
            ...
            if current["restore_reconciliation_required"] and request.restore_budget_reviewed:
                has_metered = any(
                    self.connections.get(route.connection_id)["billing"] in {"metered", "unknown"}
                    for route in [
                        request.manager,
                        request.specialist,
                        *request.role_routes.values(),
                ...
                if has_metered and (not request.run_budget_usd or not request.tender_budget_usd):
                    raise ValueError(
                        "Enter new paid-work budgets after reviewing spending since the restored backup."
                    )
                conn.execute(
            ...
            data["_connection_versions"] = {
                identifier: self.connections.get(identifier)["revision"]
                for identifier in request.allowed_connection_ids
            }
            stamp = now()
            conn.execute(
                "INSERT INTO tender_ai_policy VALUES(?,?,?,?) ON CONFLICT(tender_id) DO UPDATE SET revision=excluded.revision,data_json=excluded.data_json,updated_at=excluded.updated_at",
                (tender_id, current["revision"] + 1, dump(data), stamp),
            )
            conn.execute(
                "INSERT INTO decisions VALUES(?,?,?,?,?,?,?)",
                (
    ...
    def _routes_for(self, conn, tender_id, *, role=None):
        ...
        route = (
            (policy["role_routes"].get(role) or policy["specialist"] or policy["manager"])
            if role
            else policy["manager"]
        )
        ...
        for proposed in [route, *fallbacks]:
            if proposed and self.connections.get(proposed["connection_id"])[
                "revision"
            ] != versions.get(proposed["connection_id"]):
                raise ValueError(
                    "An AI connection changed. Review and save this tender's allowed data routes again before starting the Manager."
                )
        if not route:
            raise ValueError(
                "Open AI setup for this tender and choose a Manager model, allowed connections and budgets."
            )
        return [
            self.validate_route(r, policy["allowed_connection_ids"]) for r in [route, *fallbacks]
        ]
    ...
    def context_catalog(self, tender_id):
        ...
        selected = {
            (route["connection_id"], route["model_id"])
            for route in [policy["manager"], policy["specialist"], *policy["role_routes"].values()]
            if route
        }
        result = []
        for connection in self.connections.list():
            ...
            models.sort(
                key=lambda model: (
                    (connection["id"], model["model_id"]) not in selected,
        ...
        with self.repo.db.connect() as conn:
            return [
                ...
                for r in conn.execute(
                    "SELECT * FROM ai_usage WHERE tender_id=? ORDER BY created_at DESC",
                    (tender_id,),
                )
            ]
    ...
    def reconcile(self, tender_id, usage_id, values):
        ...
        with self.repo.atomic() as conn:
            row = conn.execute(
                "SELECT * FROM ai_usage WHERE id=? AND tender_id=?", (usage_id, tender_id)
            ).fetchone()
            if row is None:
            ...
            data = saved["data"] | {
                ...
                "detail": "Engineer reconciliation: " + request.rationale,
            }
            conn.execute("UPDATE ai_usage SET data_json=? WHERE id=?", (dump(data), usage_id))
            conn.execute(
                "INSERT INTO decisions VALUES(?,?,?,?,?,?,?)",
                (new_id(), tender_id, "ai_usage", usage_id, "reconcile", request.rationale, now()),
            )
        return {"ok": True}
...
class BudgetMeter:
    def __init__(self, policy, tender_id, run_id, route, *, metadata=None):
        ...
        self.model = next(
            m
            for m in policy.connections.models(route["connection_id"])
            if m["model_id"] == route["model_id"]
        )
    ...
    async def before_request(self, estimated_input, max_output, requests=1):
        with self.service.connections.authority_guard():
            ...
            if (
                allows_extras(self.connection)
                and policy.get("provider_managed_extras", {}).get(self.connection["id"])
                != self.connection["revision"]
            ):
                raise ValueError(
                    "Grok extra spending has not been approved for this Tender and account revision. Review the Tender's AI setup before continuing."
                )
            if (metered or allows_extras(self.connection)) and policy[
                "restore_reconciliation_required"
            ]:
                raise ValueError(
                    "Paid AI work is paused after restoring this workspace. Review spending in your provider accounts, then explicitly review and save the Tender's AI limits, including its recorded spending and reservations."
                )
            prices = self.model.get("pricing")
            if metered and (
                not prices or not policy["run_budget_usd"] or not policy["tender_budget_usd"]
            ):
                raise ValueError(
                    "This paid AI needs confirmed prices and a Tender spending allowance. Open AI setup to finish these steps."
                )
            ...
            with self.repo.atomic() as conn:
                if any(
                    ...
                    raise ValueError(
                        "The provider exceeded a reviewed online-research or native-tool allowance. Later root work is stopped for review."
                    )
                total, held, _ = self.service._totals(conn, self.tender_id)
                run_total, run_held, request_count = self.service._totals(
                    conn, self.tender_id, self.run_id
                )
                ...
                tender_left = (
                    Decimal(str(policy["tender_budget_usd"])) - total - held if metered else None
                )
                run_left = (
                    Decimal(str(policy["run_budget_usd"])) - run_total - run_held
                    if metered
                    else None
                )
    ...
    async def on_response(self, usage, reservation_id):
        ...
        with self.service.connections.authority_guard(), self.repo.db.connect(write=True) as conn:
            row = conn.execute(
                "SELECT data_json FROM ai_usage WHERE id=? AND run_id=?",
                (reservation_id, self.run_id),
            ).fetchone()
            if not row:
            ...
            search_overrun = (
                data.get("search_allowance_overrun") is True or observed_search > reserved_search
            )
            data.update(
                actual_model=usage.get("actual_model"),
                native_call_limit_overrun=(
                    data.get("native_call_limit_overrun") is True
                    or usage.get("native_call_limit_overrun") is True
                ),
                ...
                detail="Provider token usage recorded; cost is an estimate, not an invoice."
                if complete
                else "Provider usage was unavailable. The metered allowance remains reserved.",
            )
            ...
            if self.connection["protocol"] == "grok_build":
                data["detail"] = (
                    ...
                    + (" Some usage details are incomplete." if not complete else "")
                )
        ...
        self.repo.event(
            ...
            "AI usage recorded.",
            data
            | {"connection_id": self.route["connection_id"], "model_id": self.route["model_id"]},
        )
    ...
    def interrupted(self, error=None):
        ...
        with self.service.connections.authority_guard(), self.repo.db.connect(write=True) as conn:
            for identifier in self.pending:
                row = conn.execute(
                    "SELECT data_json FROM ai_usage WHERE id=?", (identifier,)
                ).fetchone()
                if row:
                    data = json.loads(row[0])
                    if rejected:
                        ...
                        data.update(
                            ...
                            if data.get("reserved_search_calls", 0)
                            else data.get("search_usage_complete", True),
                            search_usage_uncertain=bool(data.get("reserved_search_calls", 0))
                            or data.get("search_usage_uncertain", False),
                            detail="The request ended without complete usage. Reserved cost and search allowance remain until reconciled.",
                        )
                    conn.execute(
                        "UPDATE ai_usage SET data_json=? WHERE id=?", (dump(data), identifier)
                    )
