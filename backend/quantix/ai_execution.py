...
async def execute_api(route, connection, credentials, context, instruction, output_type,
                      *, consult=None, before_request=None, on_response=None,
                      definitions=None, operation="execute", system_instructions=None,
    ...
    if system_instructions is None:
        bounded_instruction = f"{INSTRUCTIONS}{TURN_CONTEXT_MARKER}Engineering work request:\n{instruction}"
    else:
        bounded_instruction = f"{system_instructions}{TURN_CONTEXT_MARKER}{instruction}" if system_instructions else instruction
    if is_subscription_profile(connection):
        ...
        return await AIWorkerClient(context.repo).execute(
            route, connection, credentials, context,
            bounded_instruction, output_type,
    ...
    return await DirectAPIService(context.repo).execute(
        ...
        bounded_instruction, output_type,
        consult=consult, before_request=before_request, on_response=on_response,
        definitions=definitions, operation_name=operation, validate_output=validate_output,
    )
