...
class ExtractionService:
    ...
    def reprocess(
        ...
        *,
        ...
        limit = successful_pages if successful_pages is not None else (
            request.page_limit if request else None
        )
        segments = []
        extracted = 0
        exceptions = 0
        ...
        for index in range(total):
            ...
            try:
                ...
                exceptions += 1
                segments.append(
                    {"page": index + 1, "state": "embedded_error", "text": "", "detail": type(error).__name__}
                )
                continue
            if text.strip():
                extracted += 1
            ...
            try:
                check_cancelled()
                png = render_page_png(
                    source, index + 1, home=self.repo.home, cancelled=cancelled
                )
                ...
                exceptions += 1
                segments.append(
                    {"page": index + 1, "state": "ocr_error", "text": "", "method": "ocr", "detail": type(error).__name__}
                )
                continue
            if ocr_text:
                extracted += 1
        ...
        with self.repo.atomic() as conn:
            ...
            conn.execute(
                """INSERT INTO extraction_versions(
                    id,original_hash,reader_id,reader_version,extracted_pages,exception_pages,
