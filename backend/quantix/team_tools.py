...
def team_tools(repo, tender_id: str, run_id: str) -> list:
    ...
    def manager(ctx) -> OfficeContext:
        ...
        if (
            ...
            or context.tender_id != tender_id
            or context.assignment_id
        ):
    ...
    def models():
        ...
        return [
            {
                "model": f"{account['connection_id']}/{model['model_id']}",
                "account": account["name"],
                "billing": account["billing"],
                "images": model["capabilities"].get("images"),
                "context_window": model["capabilities"].get("context_window"),
            }
            for account in AIPolicyService(repo).context_catalog(tender_id)
    ...
    async def hire_staff(
        ...
        repo.event(
            run_id,
            "staff_hired",
            f"The Tender Manager added {member.name} ({member.role}) to the team.",
            {"staff_id": member.id},
        )
    ...
    async def assign_work(
        ...
        if model:
            ...
            if not connection_id or not model_id:
                raise ToolArgumentError(
                    "Choose model as 'account/model' from list_team's available_models."
                )
            try:
                route = policies.validate_route(
                    route | {"connection_id": connection_id, "model_id": model_id},
                    policies.get(tender_id)["allowed_connection_ids"],
                )
            except (KeyError, ValueError) as error:
                raise ToolArgumentError(
                    f"{error} Choose one of list_team's available_models."
                ) from None
        ...
        try:
            assignment = team.assign(
                ...
                expected_result=expected_result.strip()[:2000],
                source_ids=list(source_ids or [])[:50],
                route=route,
            )
        ...
        repo.event(
            run_id,
            "work_assigned",
            f"The Tender Manager assigned {member.name}: {assignment.title}",
    ...
    async def list_team(ctx: ToolContext[OfficeContext]) -> str:
        ...
        manager(ctx)
        staff = [
            member.model_dump(include={"id", "name", "role", "specialisms", "status"})
            for member in team.list_staff(tender_id)
        ]
        assignments = [
            assignment.model_dump(include={"id", "staff_id", "title", "status", "question"})
            for assignment in team.list(tender_id, limit=40)
        ]
        return encoded(
            {"staff": staff, "recent_assignments": assignments, "available_models": models()}
        )
