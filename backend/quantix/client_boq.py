...
class ClientBoqInput(ClientBoqModel):
    ...
    def explicit_quantity_decision(self):
        if any(mapping.quantity_column for mapping in self.mappings) and not self.quantity_mapping_approved:
            raise ValueError("Changing a quantity column requires explicit quantity mapping approval.")
...
def _xml(payload):
    if len(payload) > MAX_XML_BYTES or re.search(br"<!DOCTYPE|<!ENTITY", payload, re.I):
        raise ValueError("The workbook XML is too large or contains unsupported entity declarations.")
    if payload.startswith((b"\xff\xfe", b"\xfe\xff")) or b"\x00" in payload[:100]:
        raise ValueError("This workbook uses an XML encoding that cannot be edited while preserving its original bytes.")
    try:
...
def _archive(payload):
    try:
        ...
        raise ValueError("The workbook is encrypted or is not a readable XLSX/XLSM package. Supply an unencrypted copy.") from exc
    ...
    if any(name.lower().startswith("_xmlsignatures/") for name in names):
        archive.close()
        raise ValueError("This workbook has a package digital signature. Provide an unsigned working copy before pricing it.")
    return archive
...
def _read(archive, member):
    try:
        ...
        raise ValueError("An original workbook package entry is damaged, encrypted or uses unsupported compression.") from exc
...
def _parts(archive):
    ...
    if workbook.find(f"{{{SHEET_NS}}}workbookProtection") is not None:
        raise ValueError("The source workbook is protected. Supply an authorised unprotected working copy.")
    links = {}
...
def _sheet(archive, member, strings):
    ...
    if root.find(f"{{{SHEET_NS}}}sheetProtection") is not None:
        raise ValueError("A selected worksheet is protected. Supply an authorised unprotected working copy.")
    cells = {}
    for element in root.findall(f"{{{SHEET_NS}}}sheetData/{{{SHEET_NS}}}row/{{{SHEET_NS}}}c"):
        ...
        cells[coordinate] = {"value": value, "kind": kind, "formula": formula.text if formula is not None else None, "formula_attributes": dict(formula.attrib) if formula is not None else {}}
    merged = [range_boundaries(item.get("ref")) for item in root.findall(f"{{{SHEET_NS}}}mergeCells/{{{SHEET_NS}}}mergeCell")]
...
def _header_sources(repo, tender_id, artifact_id, mapping, sheet, first_row):
    ...
    for identifier in mapping.source_ids:
        ...
        if source["artifact_id"] != artifact_id or source.get("sheet") != mapping.sheet:
            raise ValueError("Column mapping evidence must come from the selected workbook and sheet.")
        for cell in source.get("metadata", {}).get("cells", []):
            ...
            text = cell.get("value")
            if column in columns and row < first_row and isinstance(text, str) and text.strip() and not cell.get("formula"):
                if str(actual.get("value", "")).strip() != text.strip() or actual.get("formula") is not None:
                    raise ValueError("The mapped column header differs from its saved source evidence.")
                found.add(column)
    if found != columns:
        raise ValueError("Select source header evidence above the chosen rows for every mapped rate, amount and quantity column.")
...
def prepare_client_boq(repo, tender_id, request, estimate):
    ...
    if estimate["refresh_required"]:
        raise ValueError("Refresh the BOQ candidates after source changes before making a client-format copy.")
    ...
    with _archive(payload) as archive:
        ...
        suffix = ".xlsm" if Path(artifact["name"]).suffix.lower() == ".xlsm" or any(name.lower().endswith("/vbaproject.bin") for name in archive.namelist()) else ".xlsx"
        loaded, targets = {}, set()
        for mapping in request.mappings:
            ...
            for identifier in mapping.item_ids:
                ...
                if item is None or item["artifact_id"] != artifact["id"] or item["sheet"] != mapping.sheet:
                    raise ValueError("Select current BOQ rows from the mapped source workbook and sheet.")
                if not item["confirmed"] or item["unit_rate"] is None or item["rate_ex_vat"] is None or item["line_ex_vat"] is None:
                    raise ValueError("Every selected row must have confirmed source quantities and an approved price.")
                if item["currency"] != request.currency:
                    raise ValueError("Every selected row must use the explicitly reviewed workbook currency.")
                if item["tax_basis"] == "unknown" or item["vat_percent"] is None:
                    raise ValueError("Establish the selected rows' VAT treatment before writing the client workbook.")
                source = repo.get_evidence(tender_id, item["source_id"])
                if source["artifact_id"] != artifact["id"] or source.get("sheet") != mapping.sheet:
                    raise ValueError("The selected BOQ row source differs from the mapped workbook sheet.")
                row = source.get("metadata", {}).get("row")
                if not isinstance(row, int) or isinstance(row, bool) or not 1 <= row <= 1048576:
                    raise ValueError("A selected BOQ source does not identify one original worksheet row.")
                ...
                if actual_quantity.get("kind") not in {"n", None} or _number(actual_quantity.get("value"), "The original quantity") != supplied:
                    raise ValueError("The original quantity differs from the confirmed saved source quantity.")
                if mapping.quantity_column and mapping.quantity_column != quantity_column:
                    raise ValueError("Quantity mapping must name the actual confirmed source quantity column.")
                used = _number(item["effective_quantity"], "The quantity used for pricing")
                if used != supplied and not mapping.quantity_column:
                    raise ValueError("An approved measured quantity differs from the original. Explicitly approve its actual quantity-column mapping or retain the supplied quantity basis.")
                ...
                if {mapping.rate_column, mapping.amount_column} & {quantity_column, unit_column}:
                    raise ValueError("Price columns must not overwrite supplied quantity or unit cells.")
                rate_cell, amount_cell = mapping.rate_column + str(row), mapping.amount_column + str(row)
                with localcontext() as context:
                    context.prec = 50
                    unit_rate = _number(item["rate_ex_vat"], "The rate excluding VAT")
                    if request.tax_basis == "including_vat":
                        unit_rate = _number(item["unit_rate"], "The approved rate") if item["tax_basis"] == "including_vat" else unit_rate * (Decimal(1) + _number(item["vat_percent"], "VAT percent") / 100)
                amount = item["line_inc_vat"] if request.tax_basis == "including_vat" else item["line_ex_vat"]
                planned = [(rate_cell, format(unit_rate, "f"), "rate"), (amount_cell, format(_number(amount, "The line amount"), "f"), "amount")]
                ...
                for coordinate, value, role in planned:
                    ...
                    if any(left <= index <= right and top <= target_row <= bottom for left, top, right, bottom in sheet["merged"]):
                        raise ValueError("A mapped price or quantity cell is merged. Resolve the mapping in an authorised working copy.")
                    before = sheet["cells"].get(coordinate, {})
                    formula = before.get("formula")
                    if before.get("formula_attributes") or formula is not None:
                        if role != "amount" or before.get("formula_attributes") or not _ordinary_amount_formula(formula, item["quantity_cell"], rate_cell):
                            raise ValueError("A mapped target contains a supplied formula that cannot be preserved with the reviewed quantity and rate columns. It has not been overwritten or repaired.")
                        with localcontext() as context:
                            context.prec = 100
                            product = used * unit_rate
                        if not re.sub(r"\s+", "", formula).upper().startswith("ROUND(") and product != _number(value, "The approved line amount"):
                            raise ValueError("A supplied amount formula does not round the mapped rate and quantity to the approved line amount. It has been preserved; resolve the rounding rule in an authorised working copy.")
                    elif before.get("kind") in {"s", "inlineStr", "str", "e", "b", "d"} and str(before.get("value") or "").strip():
                        raise ValueError("A mapped target contains text, an error or a non-price value. Review the original column mapping.")
                    changes.append({"sheet": mapping.sheet, "member": member, "cell": coordinate, "role": role, "value": value, "item_id": identifier, "source_id": item["source_id"], "previous_value": before.get("value"), "preserved_formula": formula})
        ...
        if suffix == ".xlsm":
            warnings.append("Supplied VBA and workbook objects are preserved. No macro was executed or validated.")
    ...
    return {"artifact_id": artifact["id"], "source_name": artifact["name"], "source_hash": artifact["content_hash"], "source_version": artifact["version"], "suffix": suffix, "changes": changes, "source_ids": sorted(references), "selected_items": selected, "selected_row_count": len(selected), "candidate_row_count": candidate_count, "pricing_complete": True, "currency": request.currency, "tax_basis": request.tax_basis, "warnings": warnings}
...
def _spans(payload, wanted, row_numbers=None):
    ...
    def start(name, attributes):
        ...
        node = {"name": local, "attributes": attributes, "start": position, "open_end": _tag_end(payload, position), "path": tuple(item["name"] for item in stack) + (local,)}
        node["row_number"] = attributes.get("r") if local == "row" else stack[-1].get("row_number") if stack else None
        stack.append(node)
    ...
    def end(_name):
        ...
        if node["path"] in wanted and (row_numbers is None or node["row_number"] is None or node["row_number"] in row_numbers):
            spans.append(node)
...
def _qname(fragment):
    match = re.match(br"<([A-Za-z_][A-Za-z0-9_.:-]*)\b", fragment)
    if not match:
...
def _numeric_cell(fragment, value, children):
    ...
    opening = re.sub(br"\s+t\s*=\s*(?:\"[^\"]*\"|'[^']*')", b"", opening)
    ...
    modifications = [(node["start"], node["end"], b"") for node in children if node["name"] in {"v", "is"}]
...
def _patch_sheet(payload, changes):
    wanted = {("worksheet", "sheetData", "row"), ("worksheet", "sheetData", "row", "c"), ("worksheet", "sheetData", "row", "c", "v"), ("worksheet", "sheetData", "row", "c", "is"), ("worksheet", "sheetData", "row", "c", "extLst"), ("worksheet", "dimension")}
    ...
    for change in changes:
        ...
        if cell:
            ...
            children = [{"name": node["name"], "start": node["start"] - cell["start"], "end": node["end"] - cell["start"]} for node in spans if node["name"] in {"v", "is", "extLst"} and cell["open_end"] <= node["start"] < cell["end"]]
            ...
            following = [node for address, node in cells.items() if _coordinate(address)[1] == row_number and column_index_from_string(_coordinate(address)[0]) > column_index_from_string(column)]
            ...
            cell_xml = b"<" + prefix + b"c r=\"" + coordinate.encode("ascii") + b"\"><" + prefix + b"v>" + change["value"].encode("ascii") + b"</" + prefix + b"v></" + prefix + b"c>"
            additions.setdefault(position, []).append((column_index_from_string(column), cell_xml))
    modifications.extend((position, position, b"".join(fragment for _, fragment in sorted(values))) for position, values in additions.items())
    ...
    if dimensions:
        ...
        for change in changes:
            ...
            left, right = min(left, column_index_from_string(column)), max(right, column_index_from_string(column))
        ...
        if updated_bounds != range_boundaries(dimension["attributes"].get("ref", "A1")):
            reference = f"{get_column_letter(left)}{top}:{get_column_letter(right)}{bottom}".encode("ascii")
            fragment = payload[dimension["start"]:dimension["end"]]
            fragment = re.sub(br"\bref\s*=\s*(?:\"[^\"]*\"|'[^']*')", b'ref="' + reference + b'"', fragment)
...
def _recalculation(payload):
    ...
    if existing:
        ...
        for attribute in (b"calcMode", b"fullCalcOnLoad", b"forceFullCalc"):
            opening = re.sub(br"\s+" + attribute + br"\s*=\s*(?:\"[^\"]*\"|'[^']*')", b"", opening)
        insertion = opening.rfind(b"/>") if opening.rstrip().endswith(b"/>") else opening.rfind(b">")
        opening = opening[:insertion] + b' calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1"' + opening[insertion:]
    ...
    later = {"oleSize", "customWorkbookViews", "pivotCaches", "smartTagPr", "smartTagTypes", "webPublishing", "fileRecoveryPr", "webPublishObjects", "extLst"}
    position = min((node["start"] for node in spans if node["name"] in later), default=root_span["close_start"])
    if position < 0:
