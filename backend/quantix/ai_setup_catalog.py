...
_GROUPS = {
    ...
    "openai": (
        "openai",
        "OpenAI",
        "Choose ChatGPT/Codex subscription access or an OpenAI API key.",
    ),
...
_METHODS = {
    ...
    "openai": (
        ...
        "OpenAI charges separately for this usage; it is not included in a ChatGPT subscription.",
    ),
...
def setup_services():
    ...
    for preset in direct_provider_presets():
        ...
        if identifier not in _GROUPS:
            continue
        group_id, title, detail = _GROUPS.get(
            identifier, (identifier, preset["name"], "Connect your account for this AI service.")
        )
        group = groups.setdefault(
            group_id,
            {
                ...
                "title": title,
                "detail": detail,
                "featured": group_id in {"openai", "anthropic", "google", "xai"},
                "methods": [],
            },
        )
        label, method_detail = _METHODS.get(
            identifier,
            (
                "Use my access key",
                "Uses your account with this provider. Separately billed usage needs a Tender allowance.",
            ),
        )
        needs_details = identifier == "custom"
        group["methods"].append(
            SetupMethod(
                id=identifier,
                title=label,
                detail=method_detail,
                provider_id=identifier,
                requires_key=preset["auth_methods"][0] == "api_key",
                requires_details=needs_details,
    ...
    subscription_methods = {
        ...
        "xai": SetupMethod(
            id="grok_build",
            title="Use my Grok subscription",
    ...
    groups["openai"]["subscription_note"] = (
        "ChatGPT subscription access is provided through the original Codex client; it is separate from OpenAI API billing."
    )
    groups["openai"]["subscription_docs_url"] = "https://learn.chatgpt.com/docs/codex-sdk"
    groups["xai"]["subscription_note"] = (
        "Grok subscription access is provided through official Grok Build; any provider extras remain subject to its account settings."
    )
    groups["xai"]["subscription_docs_url"] = "https://docs.x.ai/build/overview"
    groups["anthropic"]["subscription_note"] = (
        "Claude subscription access is not offered here; use an Anthropic API key for Tender work."
    )
    groups["anthropic"]["subscription_docs_url"] = (
        "https://code.claude.com/docs/en/legal-and-compliance"
    )
    groups["google"]["subscription_note"] = (
        "Google subscription access is not offered here; use a Gemini API key. The documented consumer CLI transition and service terms do not establish a supported Quantix route."
    )
    groups["google"]["subscription_docs_url"] = (
        "https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/"
    )
    groups["custom"]["subscription_note"] = (
        "Custom connections require explicit API credentials; subscription tokens are not accepted."
    )
    order = {"openai": 0, "anthropic": 1, "google": 2, "xai": 3, "custom": 4}
    for group in groups.values():
        group["methods"].sort(
            key=lambda method: (0 if method["access_kind"] == "subscription" else 1, method["id"])
        )
    return [
        SetupService.model_validate(value).model_dump()
        for value in sorted(
            groups.values(), key=lambda item: (order.get(item["id"], 10), item["title"])
        )
    ]
...
def setup_method(service_id, method_id):
    ...
    method = (
        next((m for m in service["methods"] if m["id"] == method_id), None) if service else None
    )
