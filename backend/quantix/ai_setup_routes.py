...
def create_router(repo, setup, on_tender_ai=None):
    ...
    def manager_model(policy, manager):
        try:
            ...
            return None, None
        model = next((m for m in policy.connections.models(connection["id"]) if m["model_id"] == manager["model_id"]), None)
    ...
    def thinking(policy, tender_id):
        ...
        if not manager:
            return {"connection_id": None, "model_id": None, "current": None, "options": [], "busy": busy}
        ...
        return {"connection_id": manager["connection_id"], "model_id": manager["model_id"],
                "current": describe(saved), "options": [describe(value) for value in values], "busy": busy}
    ...
    def set_tender_thinking(tender_id: str, request: TenderThinkingInput):
        ...
        with policy.connections.authority_guard(), repo.atomic():
            ...
            manager = current["manager"]
            if not manager:
                raise ValueError("Choose the AI for this Tender first, then set how much it thinks.")
            connection, model = manager_model(policy, manager)
            if model is None:
                raise ValueError("Check this AI model in Settings first. Then return to set its thinking.")
            if request.reasoning is not None and request.reasoning not in thinking_levels(connection, model) and request.reasoning != manager["reasoning"]:
            ...
            specialist = route if current["specialist"] in (None, manager) else current["specialist"]
            label = describe(request.reasoning)["label"]
            policy.update(tender_id, {
                "allowed_connection_ids": current["allowed_connection_ids"], "manager": route, "specialist": specialist,
                "role_routes": current["role_routes"], "fallback_routes": current["fallback_routes"],
                "run_budget_usd": current["run_budget_usd"], "tender_budget_usd": current["tender_budget_usd"],
                "max_requests": current["max_requests"], "restore_budget_reviewed": False,
                "provider_managed_extras": current.get("provider_managed_extras", {}), "engineer_confirmed": True,
...
def select_tender_ai(repo, setup, tender_id: str, request: SimpleTenderAIInput):
    ...
    with policy.connections.authority_guard(), repo.atomic():
        ...
        if connection["protocol"] == "grok_build" and request.account_revision != connection["revision"]:
            raise ValueError("This Grok account or its spending choice changed. Review the current Tender confirmation before approving it.")
        if account["stage"] != "ready" or account["check"].get("model_id") != request.model_id:
            raise ValueError("Check this AI model in Settings first. Then return to choose it for this Tender.")
        ...
        if metered and reuse_budget and not current["tender_budget_usd"]:
            raise ValueError("Enter one spending allowance for this Tender before using separately billed AI.")
        route = {"connection_id": request.account_id, "model_id": request.model_id,
        ...
        specialist = current["specialist"] if current["specialist"] and current["specialist"] != current["manager"] else route
        ...
        if allows_extras(connection):
            ...
            spending_detail = "Grok subscription allowance only; pause when extra spending cannot be ruled out." if connection["protocol"] == "grok_build" else "Reviewed the displayed spending allowance."
        return policy.update(tender_id, {"allowed_connection_ids": list(dict.fromkeys([*current["allowed_connection_ids"], request.account_id])),
            "manager": route, "specialist": specialist, "role_routes": current["role_routes"], "fallback_routes": current["fallback_routes"],
            "run_budget_usd": run_budget,
            "tender_budget_usd": tender_budget, "max_requests": 32,
