...
class TeamService:
    ...
    def hire(self, tender_id: str, run_id: str, draft: StaffDraft) -> StaffMember:
        ...
        with self.repo.db.connect() as conn:
            names = {json.loads(row[0])["name"].casefold() for row in conn.execute(
                "SELECT data_json FROM team_staff WHERE tender_id=? AND status='active'", (tender_id,))}
        if draft.name.casefold() in names:
            raise ValueError(f"{draft.name} is already on this tender's team. Assign them work instead of hiring again.")
    ...
    def retire(self, tender_id: str, staff_id: str) -> StaffMember:
        ...
        with self.repo.db.connect(write=True) as conn:
            conn.execute("UPDATE team_staff SET status='retired', updated_at=? WHERE id=? AND tender_id=?",
                         (now(), staff_id, tender_id))
        return self.get_staff(tender_id, staff_id)

    def get_staff(self, tender_id: str, staff_id: str) -> StaffMember:
        with self.repo.db.connect() as conn:
            row = conn.execute("SELECT * FROM team_staff WHERE id=? AND tender_id=?", (staff_id, tender_id)).fetchone()
        if row is None:
    ...
    def list_staff(self, tender_id: str) -> list[StaffMember]:
        with self.repo.db.connect() as conn:
            rows = conn.execute("SELECT * FROM team_staff WHERE tender_id=? ORDER BY created_at, id", (tender_id,)).fetchall()
        return [self._staff(row) for row in rows]

    @staticmethod
    def _staff(row) -> StaffMember:
        return StaffMember(
            **json.loads(row["data_json"]), id=row["id"], tender_id=row["tender_id"], status=row["status"],
            portrait_seed=row["id"], created_run_id=row["created_run_id"],
            created_at=row["created_at"], updated_at=row["updated_at"],
        )

    # Assignments -----------------------------------------------------------

    def assign(self, tender_id: str, run_id: str, staff_id: str, *, title: str, brief: str,
               expected_result: str, source_ids: list[str], route: dict) -> Assignment:
        ...
        identifier, stamp = new_id(), now()
        data = {"title": title, "brief": brief, "expected_result": expected_result, "source_ids": source_ids,
                "connection_id": route["connection_id"], "model_id": route["model_id"],
                "question": None, "answer": None, "result": None, "detail": "", "usage": {}}
        with self.repo.db.connect(write=True) as conn:
            conn.execute(
    ...
    def get(self, tender_id: str, assignment_id: str) -> Assignment:
        with self.repo.db.connect() as conn:
            row = conn.execute("SELECT * FROM team_assignments WHERE id=? AND tender_id=?",
                               (assignment_id, tender_id)).fetchone()
        if row is None:
            raise KeyError("This assignment does not belong to this tender.")
        return self._assignment(row)

    def list(self, tender_id: str, *, run_id: str | None = None, staff_id: str | None = None,
             limit: int = 200) -> list[Assignment]:
        ...
        with self.repo.db.connect() as conn:
            rows = conn.execute(query + " ORDER BY created_at DESC, id LIMIT ?", (*values, limit)).fetchall()
    ...
    def complete(self, assignment: Assignment, result: AssignmentResult, usage: dict) -> Assignment:
        return self._update(assignment, "completed", result=result.model_dump(), usage=_add_usage(assignment.usage, usage))

    def ask(self, assignment: Assignment, question: str, usage: dict) -> Assignment:
        return self._update(assignment, "waiting", question=question, usage=_add_usage(assignment.usage, usage))
    ...
    def recover_interrupted(self) -> int:
        ...
        with self.repo.db.connect() as conn:
            rows = conn.execute("SELECT * FROM team_assignments WHERE status IN ('queued','running')").fetchall()
        for row in rows:
            self._update(self._assignment(row), "cancelled", detail="Quantix stopped before this work finished.")
        return len(rows)

    def _update(self, assignment: Assignment, status: str, **changes) -> Assignment:
        data = assignment.model_dump(include={"title", "brief", "expected_result", "source_ids", "connection_id",
                                              "model_id", "question", "answer", "result", "detail", "usage"})
        data.update(changes)
        with self.repo.db.connect(write=True) as conn:
            conn.execute("UPDATE team_assignments SET status=?, data_json=?, updated_at=? WHERE id=?",
    ...
    def _assignment(row) -> Assignment:
        return Assignment(
            **json.loads(row["data_json"]), id=row["id"], tender_id=row["tender_id"], run_id=row["run_id"],
            staff_id=row["staff_id"], status=row["status"], created_at=row["created_at"], updated_at=row["updated_at"],
        )


def _add_usage(current: dict, extra: dict) -> dict:
    counters = ("requests", "input_tokens", "output_tokens", "cached_input_tokens", "reasoning_tokens", "web_search_calls")
