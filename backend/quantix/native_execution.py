...
class NativeExecutionService:
    ...
    def prepare(
        self, context, connection, route, *, operation="execute", tools=(), resume_session_id=None
    ):
        ...
        with connections.authority_guard(), self.repo.atomic() as conn:
            ...
            if run["tender_id"] != context.tender_id or run["status"] not in {"queued", "running"}:
                raise ValueError(
                    "Only current active Tender work can start an original-client session."
                )
            ...
            try:
                ...
                raise ValueError(
                    "This work needs its immutable Tender Manager profile pin before starting an original-client session."
                ) from None
            profile_id, version = (
                (context.actor_id, profile.version)
                if getattr(context, "is_staff", False)
                else (profile.id, profile.version)
            )
            runtime = connection.get("_checked_component_version")
            if not runtime:
                raise ValueError(
                    "Check this exact original-client software and model before creating a managed session."
                )
            artifacts = [
                {key: artifact[key] for key in ("id", "version", "content_hash")}
                for artifact in self.repo.list_artifacts(context.tender_id)
            ]
            scope = _fingerprint(
                {
                    "artifacts": artifacts,
                    "approved_scope": getattr(context, "approved_scope", None),
                    "tools": list(tools),
                    "operation": operation,
                }
            )
            observed_model = connection.get("_model") or {}
            settings = _fingerprint(
                {
                    "route": route,
                    "model": {
            ...
            if (
                ...
                if origin is not None:
                    candidates = conn.execute(
                        "SELECT data_json FROM native_client_sessions WHERE tender_id=? AND run_id=? AND connection_id=? ORDER BY updated_at DESC",
                        (context.tender_id, origin[0], connection["id"]),
                    ).fetchall()
                    fields = (
                        "protocol",
                        "connection_revision",
                    ...
                    for candidate_row in candidates:
                        ...
                        if (
                            ...
                            and candidate.state in {"completed", "interrupted", "running"}
                            and all(
                                getattr(candidate, name) == getattr(binding, name)
                                for name in fields
                            )
                        ):
            ...
            if resume_session_id:
                row = conn.execute(
                    "SELECT data_json FROM native_client_sessions WHERE tender_id=? AND id=?",
                    (context.tender_id, resume_session_id),
                ).fetchone()
                if row is None:
                ...
                if previous.state == "running" and self.repo.get_run(previous.run_id)["status"] in {
                    "failed",
                    "cancelled",
                    "interrupted",
                }:
                ...
                if (
                    previous.state not in {"completed", "interrupted"}
                    or not previous.provider_session_id
                    or any(getattr(previous, name) != getattr(binding, name) for name in fields)
                ):
                    raise ValueError(
                        "The original-client session is incompatible with the current profile, account, model, runtime or reviewed scope. Start a new session."
                    )
                if previous.protocol == "grok_build":
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Grok continuation needs its exact native UUID. Start a new session instead of matching a title."
                        ) from None
                binding = binding.model_copy(
                    update={"id": previous.id, "provider_session_id": previous.provider_session_id}
                )
                conn.execute(
                    "UPDATE native_client_sessions SET run_id=?,state=?,data_json=?,updated_at=? WHERE id=?",
                    (context.run_id, "prepared", dump(binding.model_dump()), now(), previous.id),
                )
            else:
                conn.execute(
                    "INSERT INTO native_client_sessions VALUES(?,?,?,?,?,?,?,?)",
                    (
                        binding.id,
                        binding.tender_id,
                        binding.run_id,
            ...
            conn.execute(
                "INSERT INTO native_client_session_turns VALUES(?,?,?,?,?,?,?)",
                (
                    new_id(),
                    binding.id,
                    binding.run_id,
    ...
    def finish(self, context, binding_id, *, provider_session_id=None, state="completed"):
        ...
        if provider_session_id is not None and (
            not isinstance(provider_session_id, str) or not _SAFE_ID.fullmatch(provider_session_id)
        ):
            raise ValueError("The original client returned an invalid session identifier.")
        with self.repo.atomic() as conn:
            row = conn.execute(
                "SELECT data_json FROM native_client_sessions WHERE tender_id=? AND id=? AND run_id=?",
                (context.tender_id, binding_id, context.run_id),
            ).fetchone()
            if row is None:
            ...
            if (
                saved.state in {"completed", "interrupted", "failed", "incompatible"}
                and state == "running"
            ):
            ...
            if (
                ...
                and saved.provider_session_id != provider_session_id
            ):
                raise ValueError("The original client changed its session identity unexpectedly.")
            updated = saved.model_copy(
                update={
                    "state": state,
                    "provider_session_id": provider_session_id or saved.provider_session_id,
                }
            )
            conn.execute(
                "UPDATE native_client_sessions SET state=?,data_json=?,updated_at=? WHERE id=?",
                (state, dump(updated.model_dump()), now(), binding_id),
            )
            conn.execute(
                "UPDATE native_client_session_turns SET state=?,binding_json=?,updated_at=? WHERE session_id=? AND run_id=? AND state IN ('prepared','running')",
...
def project_client_event(context, kind, data, *, request_operation=None):
    ...
    with context.repo.atomic():
        ...
        if request_operation:
            ...
            phase = (
                "completed" if data.get("complete") else "observed" if settled_section else "delta"
            )
            capture_status = (
                "partial" if settled_section and not data.get("complete") else "complete"
            )
            ...
            if operation is None:
                operation = recorder.start(
                    ...
                    else "Original client response draft.",
                    values,
                    phase=phase if settled_section else "started",
                    parent_operation_id=request_operation,
