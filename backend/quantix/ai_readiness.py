...
def _ready_models(repo, connection):
    ...
    with repo.db.connect() as conn:
        if not conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='ai_setup_accounts'").fetchone():
            return {}, None
        row = conn.execute("SELECT data_json FROM ai_setup_accounts WHERE connection_id=?", (connection["id"],)).fetchone()
        if not row:
            return {}, None
        ...
        if conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='ai_setup_model_checks'").fetchone():
            for model_row in conn.execute("SELECT model_id,data_json FROM ai_setup_model_checks WHERE connection_id=?", (connection["id"],)):
                records[model_row["model_id"]] = json.loads(model_row["data_json"])
    records = {model_id: value for model_id, value in records.items()
               if value.get("check", {}).get("model_id") == model_id and value["check"].get("status") == "passed"
    ...
    return {model_id: value for model_id, value in records.items()
            if value.get("checked_component_version") == version}, selected
...
def require_ready(repo, connection, model_id):
    ...
    if evidence is None:
        raise ValueError("Finish this AI account's connection check in Settings before starting Tender work.")
