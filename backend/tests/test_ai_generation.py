...
def test_explicit_sampling_reaches_sdk_and_absent_preserves_default():
    ...
    settings = build_model_settings(
        {"model_id": "exact-test-model", "temperature": 0.4, "top_p": 0.8}, connection
    )
    assert settings["temperature"] == 0.4 and settings["top_p"] == 0.8
...
def test_unknown_explicit_sampling_fails_instead_of_silently_dropping():
    ...
    with pytest.raises(ValueError, match="temperature"):
        build_model_settings({"model_id": "exact-test-model", "temperature": 0.4}, account())
...
def test_client_sampling_is_explicitly_unavailable():
    ...
    with pytest.raises(ValueError, match="temperature"):
        validate_generation({"temperature": 0.4}, account("codex", "codex", temperature=True))
...
def test_preview_never_exposes_connection_private_fields():
    ...
    result = generation_preview(connection, {"temperature": 0.5}).model_dump_json()
...
def test_absent_controls_preserve_historical_route_identity():
    ...
    assert (
        AIRoute.model_validate({**historical, "temperature": 0.5}).model_dump()["temperature"]
        == 0.5
    )
...
def test_sdk_sampling_rejection_is_not_a_silent_default():
    ...
    with pytest.raises(ValueError, match="reasoning"):
        validate_sdk_settings(
            {"temperature": 0.2},
            {"openai_supports_reasoning": True, "openai_reasoning_enabled_by_default": True},
        )
    with pytest.raises(ValueError, match="sampling"):
        validate_sdk_settings({"top_p": 0.8}, {"anthropic_disallows_sampling_settings": True})
...
def test_openai_search_uses_provider_enforced_total_native_call_limit():
    ...
    settings = build_model_settings(
        {"web_search": True, "max_search_calls": 2}, account(web_search=True)
    )
...
async def test_uncapped_search_denied_before_model_transport_or_budget_reservation(
    ...
    with pytest.raises(ValueError, match="enforce|call limit"):
        await run_model(
            ...
            account(protocol, provider, web_search=True),
            {},
            None,
            "Search",
            Output,
            definitions=[],
            before_request=forbidden,
            on_response=forbidden,
        )
...
async def test_reported_search_calls_above_shared_native_cap_are_withheld_and_uncertain():
    ...
    def response(messages, info):
        return ModelResponse(
            ...
            model_name="exact-test-model",
            usage=RequestUsage(input_tokens=100, output_tokens=20),
        )

    model = MeteredModel(
        FunctionModel(response, model_name="exact-test-model"),
        route={
            "model_id": "exact-test-model",
            "web_search": True,
            "max_output_tokens": 1024,
        ...
        connection=account(web_search=True, context_window=2000),
        context=None,
        before_request=lambda *_: "held",
        on_response=lambda usage, _: reported.append(usage),
    )
    with pytest.raises(ValueError, match="exceeded.*native-call"):
        await model.request(
            [ModelRequest(parts=[UserPromptPart("Search")])], {}, ModelRequestParameters()
        )
...
async def test_real_agent_loop_applies_settings_validates_output_and_records_each_request(
    monkeypatch, mode
):
    ...
    def response(messages, info):
        ...
        parts = (
            [ToolCallPart(info.output_tools[0].name, {"value": 42})]
            if info.output_tools
            else [TextPart('{"value":42}')]
        )
        return ModelResponse(
            parts=parts,
            model_name="exact-test-model",
    ...
    async def binding(route, connection, credentials):
        yield APIModelBinding(
            ...
            build_model_settings(route, connection),
            None,
        )
    ...
    result = await run_model(
        ...
        account(temperature=True, structured_output=True),
        {},
        None,
        "Return 42.",
        Result,
    ...
    assert observed[0]["temperature"] == 0.4 and observed[0]["max_tokens"] == 1024
...
def test_generation_preview_endpoint_is_source_backed_and_does_not_change_account(tmp_path):
    ...
    with TestClient(app) as client:
        response = client.post(
            f"/api/ai/connections/{saved['id']}/generation-preview",
            json={"model_id": "exact-test-model", "settings": {"temperature": 0.4}},
        )
        assert response.status_code == 200
        assert response.json()["effective"]["temperature"] == 0.4
