...
def test_unknown_optional_price_and_streaming_do_not_change_old_saved_shapes():
    ...
    with pytest.raises(ValidationError):
        PriceCard(
            ...
            source="fixture",
            as_of="2026-09-13",
            code_execution_per_session=0.5,
        )
...
def test_price_only_edit_preserves_observed_capability_origin(tmp_path):
    ...
    account = service.create(
        {
            ...
            "auth_type": "api_key",
            "credentials": {"api_key": "fixture"},
            "session_only": True,
        }
    )
    model = {
        "model_id": "synthetic-exact",
    ...
    changed = service.save_model(
        account["id"], {**model, "capabilities": {"tools": True, "images": False}, "pricing": price}
    )
...
def test_light_route_clears_search_without_changing_engineering_preferences():
    ...
    assert (
        build_model_settings(classified, connection)["anthropic_thinking"]["budget_tokens"] == 8192
    )
