...
def test_http_sheet_and_range_filter_before_pagination_and_preserve_artifact_scope(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        result = client.get(
            path, params={"sheet": "Rates", "cell_range": "B120:D125", "offset": 1, "limit": 2}
        )
        ...
        assert (
            client.get(
                path.replace(tender["id"], other["id"]), params={"sheet": "Rates"}
            ).status_code
            == 404
        )
...
@pytest.mark.parametrize(
    "region", ["not-a-range", "A0:D2", "XFE1:XFE2", "A1048577:B1048578", "D20:A1"]
)
def test_invalid_sheet_range_is_actionable_before_reading_sources(tmp_path, region):
    ...
    with TestClient(app) as client:
        client.headers["Authorization"] = "Bearer synthetic-session"
        response = client.get(
            f"/api/tenders/{tender['id']}/artifacts/{artifact['id']}/evidence",
            params={"sheet": "BOQ", "cell_range": region},
        )
