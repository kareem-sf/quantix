...
def test_unnamed_tender_takes_its_package_name_and_queues_identification(
    ...
    monkeypatch.setattr(
        JobManager, "start_analysis", lambda self, tender_id: identified.append(tender_id)
    )
...
def test_identification_run_publishes_the_project_name(client, tmp_path, monkeypatch):
    ...
    async def identify(repo, tender_id, run_id):
        return PreparedIdentityResult(
            tender_id, run_id, ProjectIdentity(name="Harbour Road Upgrade Phase 2"), {"requests": 1}
        )
...
def test_without_ai_access_the_package_is_analysed_locally_and_named_provisionally(
    client, tmp_path
):
    tender = client.post("/api/tenders", json={}).json()
    run = client.post(
...
def test_failed_identification_is_visible_and_names_the_tender_provisionally(
    client, tmp_path, monkeypatch
):
    from quantix import project_identity
    from quantix.jobs import JobManager
    ...
    messages = client.get(f"/api/tenders/{tender['id']}/messages").json()
    assert any(
        "could not finish analyzing the tender package" in message["content"]
...
def test_engineer_rename_is_protected_from_identification(client):
    ...
    result = apply_identity(
        repo, PreparedIdentityResult(tender["id"], run["id"], ProjectIdentity(name="Other"), {})
    )
...
def test_identification_keeps_proposed_facts_separate_from_reviewed_profile(tmp_path):
    ...
    profiles.update(
        ...
        ProfilePatch(expected_revision=1, idempotency_key="seed", country="Egypt"),
    )
    run = repo.create_run(tender["id"], "identify")
    identity = ProjectIdentity(
        name="New Administrative Capital Water Network",
        client="NUCA",
        location="Cairo",
        country="Saudi Arabia",
        contract_type="FIDIC Red Book",
        currencies=["egp", "usd"],
        submission_deadline="12 October 2026, 12:00",
        summary="Supply and installation of a potable water network.",
        sources=["01 ITT/Invitation.pdf"],
    )
    with repo.atomic():
        result = apply_identity(
            repo, PreparedIdentityResult(tender["id"], run["id"], identity, {"requests": 1})
        )
    saved = repo.get_tender(tender["id"])
    assert (saved["name"], saved["name_source"]) == (
        "New Administrative Capital Water Network",
        "ai",
    )
...
def test_new_tender_creation_does_not_grant_ai_authority(client, monkeypatch, previous_approval):
        ...
        with repo.db.connect(write=True) as conn:
            conn.execute(
                ...
                (
                    ...
                    json.dumps(
                        {
                            "allowed_connection_ids": ["acct"],
                            "manager": {"connection_id": "acct", "model_id": "model"},
                            "run_budget_usd": 1.5,
                            "tender_budget_usd": 2.0,
                            "provider_managed_extras": {"acct": 1},
                        }
                    ),
    ...
    grants = []
    monkeypatch.setattr(
        AIPolicyService, "update", lambda self, tender_id, values: grants.append(values)
    )
    monkeypatch.setattr(
        AISetupService,
        "list",
    ...
    monkeypatch.setattr(
        "quantix.ai_setup_routes.select_tender_ai",
        lambda *args, **kwargs: grants.append("selection"),
    )
    created = client.post("/api/tenders", json={})
...
def test_identification_runs_pin_the_manager_profile(tmp_path):
    ...
    tender = repo.create_tender()
    with repo.atomic():
        run = JobManager(repo, object())._queue(
            tender["id"], "identify", "Identify the project from its package."
        )
...
async def test_subscription_route_reserves_without_a_usd_allowance(tmp_path, monkeypatch):
    ...
    repo, tender, _connections, _connection, _model, policy, route = configured_office(
    ...
    policy.update(
        tender["id"],
        {
            ...
            "rationale": "Subscription route without a USD allowance",
        },
    )
