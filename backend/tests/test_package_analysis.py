...
def _tender(tmp_path, *, name=None):
    ...
    evidence = [
        (
            "01 ITT/Invitation to Tender.pdf",
            "pdf",
            "Invitation to tender for the Harbour Road Upgrade, Alexandria.",
        ),
    ...
    for index, (path, kind, text) in enumerate(evidence):
        repo.register_artifact(
            ...
            len(text),
            {
                ...
                "warnings": [],
                "segments": [
                    {"locator": "page:1", "text": text, "page": 1, "kind": "text", "metadata": {}}
                ],
            },
        )
    return repo, tender
...
class _Stub:
    ...
    async def __call__(self, repo, tender_id, run_id, prompt, output_type, *, operation):
        ...
        if output_type is BriefBatch:
            ids = [
                artifact["id"]
                for artifact in repo.list_artifacts(tender_id)
                if artifact["id"] in prompt
            ]
            return BriefBatch(
                briefs=[
                    DocumentBrief(
        ...
        return PackageOverview(
            identity=ProjectIdentity(
                name="Harbour Road Upgrade",
                client="Alexandria Port Authority",
                submission_deadline="12 October 2026",
            ),
            overview="Road and quay works tender.",
            gaps=["No bill of quantities was found."],
        ), usage
...
async def test_local_stages_complete_and_the_map_waits_for_ai(tmp_path, local_only):
    ...
    result = await run_analysis(
        repo, tender["id"], run["id"], threading.Event(), ai_ready=lambda: "No AI chosen."
    )
...
async def test_package_map_names_the_project_and_briefs_are_cached(
    tmp_path, local_only, monkeypatch
):
    repo, tender = _tender(tmp_path)
    stub = _Stub()
    ...
    run = repo.create_run(tender["id"], "analysis")
    result = await run_analysis(
        repo, tender["id"], run["id"], threading.Event(), ai_ready=lambda: True
    )
    ...
    assert (
        "**Harbour Road Upgrade**" in summary and "Submission deadline: 12 October 2026" in summary
    )
    ...
    second = await run_analysis(
        repo, tender["id"], rerun["id"], threading.Event(), ai_ready=lambda: True
    )
...
def test_recognition_summary_counts_pages_needing_a_second_reading():
    artifacts = [
        {
            "relative_path": "scans/drawing.pdf",
            "metadata": {"page_count": 4, "ocr_pages": 3},
            "warnings": [
                {"code": "ocr_low_confidence", "locator": "page:2"},
                {"code": "pdf_no_text", "locator": "page:4"},
            ],
        }
    ]
    ...
    assert (
        summary["pages"],
        summary["recognised_pages"],
        summary["uncertain_pages"],
        summary["unreadable_pages"],
    ) == (4, 3, 1, 1)
    assert summary["attention"] == ["scans/drawing.pdf page:2", "scans/drawing.pdf page:4"]


async def test_brief_cache_is_bound_to_tender_and_complete_source_context(
    tmp_path, local_only, monkeypatch
):
    repo, tender = _tender(tmp_path)
    stub = _Stub()
    ...
    for artifact in repo.list_artifacts(tender["id"]):
        repo.register_artifact(
            ...
            20,
            {
                "kind": "pdf",
                "status": "extracted",
                "segments": [{"locator": "page:1", "text": "Different opening context."}],
            },
        )
    stub.calls.clear()
    ...
    with repo.db.connect(write=True) as conn:
        conn.execute(
            "UPDATE evidence SET text='Corrected extraction for another project.' WHERE artifact_id=?",
            (artifact["id"],),
        )
    await analyze(tender["id"])
    assert stub.calls == ["package_briefs", "package_map"]
...
async def test_staff_read_the_same_package_map_as_the_manager(tmp_path, monkeypatch):
    ...
    saved_map = {
        "current": True,
        "identity": {"client": "Synthetic employer"},
        "overview": "Two-storey school",
        "gaps": [],
        "readability": {},
        "documents": [{"document_id": artifact["id"], "brief": "Specification"}],
    }
...
def test_overlong_orientation_text_is_trimmed_not_rejected():
    brief = DocumentBrief.model_validate(
        {
            "document_id": "d1",
            "document_type": "addendum_or_clarification",
            "title": None,
            "brief": "b" * 900,
            "key_locations": ["page 1 onward: " + "x" * 300],
            "key_topics": ["t"] * 12,
        }
    )
    assert (
        len(brief.brief) == 600
        and len(brief.key_locations[0]) == 120
        and len(brief.key_topics) == 8
    )
