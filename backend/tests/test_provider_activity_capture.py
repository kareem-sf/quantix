...
async def test_direct_draft_and_summary_keep_full_sections(tmp_path):
    ...
    await DraftStreamEvents(
        context, {"provider_id": "openai", "protocol": "openai_responses"}, Output
    ).handle(None, events())
    saved = repo.run_events(run["id"])
    assert (
        "".join(e["data"]["text"] for e in saved if e["kind"] == "assistant_text_delta") == content
    )
    assert [e["data"]["text"] for e in saved if e["kind"] == "assistant_reasoning_summary"][
        -1
    ] == summary
...
async def test_client_buffer_retains_long_drafts_and_separate_summary_sections():
    ...
    await buffer.finish()
    assert (
        "".join(data["text"] for kind, data in saved if kind == "assistant_text_delta") == content
    )
    summaries = [
        data
        for kind, data in saved
        if kind == "assistant_reasoning_summary" and data.get("complete")
    ]
...
def test_summary_settings_do_not_change_reasoning_effort():
    ...
    settings = build_model_settings(
        route, {"provider_id": "openai", "protocol": "openai_responses"}
    )
    assert settings["openai_reasoning_effort"] == "high"
    assert settings["openai_reasoning_summary"] == "auto"
    google = build_model_settings(
        {**route, "model_id": "gemini-3-pro"}, {"provider_id": "google", "protocol": "google"}
    )
    assert google["google_thinking_config"] == {"thinking_level": "high", "include_thoughts": True}
    ordinary = build_model_settings(
        {"model_id": "gpt-4.1-mini", "max_output_tokens": 8192},
        {"provider_id": "openai", "protocol": "openai_responses"},
    )
    assert "openai_reasoning_summary" not in ordinary
    ordinary_google = build_model_settings(
        {"model_id": "gemini-2.0-flash", "max_output_tokens": 8192},
        {"provider_id": "google", "protocol": "google"},
    )
    ...
    with repo.db.connect() as conn:
        return [
            ...
            for row in conn.execute(
                "SELECT e.data_json,p.content FROM run_events e JOIN run_activity_payloads p ON p.event_id=e.id WHERE e.run_id=? ORDER BY e.id",
                (run_id,),
            )
        ]
...
@pytest.mark.asyncio
async def test_real_agent_records_separate_request_sections_and_canonical_input(
    ...
    async def stream(messages, info):
        ...
        yield {
            0: DeltaToolCall(
                name=info.output_tools[0].name, json_args=json.dumps({"summary": text})
            )
        }
    ...
    async def binding(*args):
        yield APIModelBinding(
            FunctionModel(stream_function=stream, model_name="fixture"), {"max_tokens": 8192}, None
        )
    ...
    await run_model(
        {"model_id": "fixture", "max_output_tokens": 8192},
        {"provider_id": "openai", "protocol": "openai_responses", "billing": "subscription"},
        {"api_key": "synthetic-secret-key"},
        context,
        "Complete supplied instruction synthetic-secret-key",
        Output,
        definitions=[],
        before_request=lambda *a: "reservation",
    ...
    prepared = [
        (meta, value)
        for meta, value in captured
        if meta["category"] == "model_request" and meta["phase"] == "prepared"
    ]
    ...
    drafts = [
        (meta, value)
        for meta, value in captured
        if meta["category"] == "draft" and meta["phase"] == "completed"
    ]
    assert len(drafts) == 2
    assert len({meta["parent_operation_id"] for meta, _ in drafts}) == 2
    assert [value["text"] for _, value in drafts] == [
        "Rejected first draft " * 5000,
        "Accepted second draft " * 5000,
    ]
...
async def test_legacy_and_durable_public_text_both_redact_known_credentials(tmp_path):
    ...
    await DraftStreamEvents(context, {}, Output, request_operation=lambda: operation).handle(
        None, events()
    )
    project_client_event(
        context,
        "assistant_reasoning_summary",
        {"text": "custom-credential-canary", "complete": True},
...
async def test_native_control_records_canonical_catalogue_and_public_sections(tmp_path):
    ...
    control = WorkerControlBridge(
        source, {"provider_id": "codex", "protocol": "codex"}, {"model_id": "fixture"}
    )

    async def emit(kind, data):
        result = await control._call_tool(
            None,
            types.CallToolRequestParams(
                name="event", arguments={"kind": kind, "message": "Fixture", "data": data}
            ),
        )
    ...
    await emit(
        "runtime_request_activity",
        {
            ...
            "phase": "prepared",
            "payload": {"instructions": "Exact supplied content"},
        },
    )
    await emit(
        "assistant_reasoning_summary",
    ...
    await emit(
        "runtime_request_activity",
        {"request_id": "request-1", "phase": "completed", "payload": {"usage": {"requests": 2}}},
    )
    captured = activity_payloads(repo, run["id"])
    prepared = captured[0][1]
    assert prepared["instructions"] == "Exact supplied content"
    assert (
        prepared["quantix_tool_catalogue"][0]["inputSchema"]["properties"]["summary"]["type"]
        == "string"
    )
    assert "Read this Tender only" in prepared["quantix_tool_instructions"]
    summary = next(
        (meta, payload) for meta, payload in captured if meta["category"] == "reasoning_summary"
    )
...
def test_stopped_native_request_can_retain_already_observed_partial_section(tmp_path):
    ...
    project_client_event(
        ...
        {"text": "Already observed draft", "delta": False, "complete": False},
        request_operation=operation,
    )
    captured = activity_payloads(repo, run["id"])
