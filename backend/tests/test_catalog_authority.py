...
def test_changed_model_invalidates_only_its_readiness_and_cannot_reinherit_proof(
    ...
    if change == "manual_denied":
        ...
        connections.store_discovered_models(
            account["id"],
            [second] if change == "removed" else [denied, second],
            account["revision"],
        )
...
def test_account_authority_edits_still_revoke_routes(tmp_path, monkeypatch, change):
    repo, tender, connections, account, model, policy, route = configured_office(
        tmp_path, monkeypatch, provider_id="custom" if change == "billing" else "openai"
    )
...
async def test_discovery_respects_explicit_provider_capability_denial(tmp_path, monkeypatch, path):
    ...
    monkeypatch.setattr(
        "quantix.ai_connections.documented_capabilities", lambda *_: {"tools": True}
    )
    if path == "setup":
