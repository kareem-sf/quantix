...
def test_static_instructions_are_separated_from_the_turn_context(monkeypatch):
    ...
    async def capture(
        self, route, connection, credentials, context, instruction, output_type, **kwargs
    ):
    ...
    for turn in (
        '{"engineer_request": "What is the bid bond?", "today_utc": "2026-09-13"}',
        '{"engineer_request": "List every submission form.", "today_utc": "2026-09-14"}',
    ):
        asyncio.run(
            execute_api({}, connection, {}, Context(), turn, dict, system_instructions=INSTRUCTIONS)
        )
