...
def _register(repo, tender_id, text, path="Sources/pump.pdf"):
    artifact, _ = repo.register_artifact(
        tender_id, path, hashlib.sha256(text).hexdigest(), len(text),
        {"kind": "pdf", "status": "extracted", "segments": [{"locator": "page:1", "text": text.decode()}]},
    )
    return artifact
...
def test_work_product_rejects_web_refs_and_derives_source_revision_impact(tmp_path):
    ...
    saved = service.save_draft(
        identity,
        WorkProductDraft(kind="comparison", title="Pump comparison", rows=[],
                         content="Compare the specified pump duty.", source_refs=[evidence["id"]],
                         method_refs=[], idempotency_key="product-dependencies"),
    )
    ...
    with pytest.raises(ValueError, match="Tender evidence IDs"):
        service.save_draft(
            identity,
            WorkProductDraft(kind="note", title="Unsafe URL", content="This must not save.",
                             source_refs=["https://model-invented.example/not-cited"],
                             idempotency_key="unsafe-public-url"),
        )

    _register(repo, tender["id"], b"Specified pump duty is 80 m3 per hour.", artifact["relative_path"])
