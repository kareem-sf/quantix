...
def test_package_blockers_identify_exact_requirement_and_output_records(workspace):
    ...
    service = submission_service(repo)
    source = estimates.view(tid)["items"][0]["source_id"]
    requirement = service.requirements.propose(
        tid,
        {
            "title": "Construction sequence",
            "detail": "Provide a programme.",
            "source_ids": [source],
    ...
    assert all(
        blocker.target.kind == "requirement" and blocker.target.record_id == requirement["id"]
    ...
    target = next(
        blocker["target"] for blocker in changed["blockers"] if blocker["code"] == "output_changed"
    )
...
def test_package_missing_linked_documents_carries_exact_selection_ids(workspace):
    ...
    requirement = service.requirements.propose(
        ...
        {
            "title": "Programme",
            "detail": "Supply the reviewed programme.",
            "source_ids": [source],
            "deliverable_kind": "programme_xlsx",
        },
    )
    ...
    service.requirements.decide(
        tid, requirement["id"], approval(decision="approve", applicability_reviewed=True)
    )
    ...
    target = next(
        blocker["target"] for blocker in missing["blockers"] if blocker["code"] == "missing_output"
    )
    assert target == {
        "kind": "package",
        "record_id": requirement["id"],
        "output_ids": [required["id"]],
    }
