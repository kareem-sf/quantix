...
def _draft(name="Samir Haddad", role="Senior Quantity Surveyor"):
    return StaffDraft(
        name=name,
        role=role,
        specialisms=["BOQ audit", "Concrete works"],
...
def office(tmp_path, monkeypatch):
    ...
    _, _, _, connection, _, policy, route = configured_office(
        tmp_path, monkeypatch, repo=repo, tender=tender
    )
...
async def test_manager_tools_hire_and_assign_and_refuse_staff_callers(office):
    ...
    hired = await _tool(tools, "hire_staff").invoke(
        manager,
        {
            "name": "Samir Haddad",
            "role": "Senior Quantity Surveyor",
            "specialisms": ["BOQ audit"],
            "background": "Pricing public buildings.",
    ...
    with pytest.raises(ToolArgumentError, match="available_models"):
        await _tool(tools, "assign_work").invoke(
            manager,
            {
                "staff_id": staff_id,
                "title": "Slab check",
                "brief": "Check the ground slab.",
                "expected_result": "Concrete grade and thickness",
                "model": "unknown/model",
            },
        )
    await _tool(tools, "assign_work").invoke(
    ...
    staff_context = OfficeContext(
        repo, tender["id"], run["id"], actor_id=staff_id, assignment_id=queued[0].id
    )
...
async def test_staff_question_then_answer_then_cited_result(office, monkeypatch):
    ...
    assignment = team.assign(
        ...
        title="Slab check",
        brief="Check the ground slab.",
        expected_result="Grade and thickness",
        source_ids=[evidence["id"]],
        route=route,
    )
    packets = []
    ...
    async def provider(route, connection, credentials, context, prompt, output_type, **kwargs):
        ...
        if len(packets) == 1:
            return {
                "output": {"kind": "question", "question": "Which slab: ground or roof?"},
        ...
        await dispatch(
            "direct", read, context, {"source_id": evidence["id"]}, invocation_id="staff-read-1"
        )
        return {
            "output": {
                "kind": "completed",
                "summary": "Ground slab is C30/37 and 250 mm.",
                "findings": [
                    {
                        "title": "Slab grade",
                        "detail": "C30/37, 250 mm.",
                        "kind": "requirement",
                        "source_ids": [evidence["id"]],
                    }
                ],
            ...
            "usage": {"requests": 2, "input_tokens": 30, "output_tokens": 12},
            "web_sources": [],
        }
...
async def test_unread_citation_or_provider_failure_fails_only_that_assignment(office, monkeypatch):
    ...
    second = team.hire(
        tender["id"], run["id"], _draft(name="Lina Farouk", role="Planning Engineer")
    )
    cited = team.assign(
        tender["id"],
        run["id"],
        first.id,
        title="Cite unread",
        brief="Report the grade.",
        expected_result="Grade",
        source_ids=[],
        route=route,
    )
    broken = team.assign(
        ...
        brief="Draft programme.",
        expected_result="Programme",
        source_ids=[],
        route=route,
    )
...
async def test_manager_hires_assigns_and_reads_the_result_in_its_next_turn(office, monkeypatch):
    ...
    async def provider(route, connection, credentials, context, prompt, output_type, **kwargs):
        ...
        if context.assignment_id:
            ...
            return {
                "output": {
                    "kind": "completed",
                    "summary": "Slab is C30/37, 250 mm.",
                    "source_ids": [evidence["id"]],
                },
                "usage": {"requests": 2},
                "web_sources": [],
            }
        manager_prompts.append(
            json.loads(prompt.split("---- Current request and context ----", 1)[-1])
        )
            ...
            hired = json.loads(
                await dispatch(
                    ...
                    context,
                    {
                        "name": "Samir Haddad",
                        "role": "Senior Quantity Surveyor",
                        "specialisms": ["Concrete"],
                        "background": "Public buildings.",
                        "working_style": "Careful.",
                    },
                    invocation_id="m-hire",
                )
            )
            await dispatch(
                "direct",
                ...
                invocation_id="m-assign",
            )
            return {
                "output": {"summary": "Samir is checking the slab."},
                "usage": {"requests": 1},
                "web_sources": [],
            }
        await dispatch(
            ...
            context,
            {"source_id": evidence["id"]},
            invocation_id="m-read",
        )
        return {
            "output": {
                "summary": "The ground slab is C30/37, 250 mm thick.",
                "source_ids": [evidence["id"]],
            },
            "usage": {"requests": 1},
            "web_sources": [],
    ...
    result = await run_manager(
        repo, tender["id"], run["id"], "What concrete grade is the ground slab?"
    )
