...
async def test_codex_uses_exact_native_resume_with_fresh_scoped_bridge(tmp_path, monkeypatch):
    ...
    class Codex:
        ...
        async def thread_resume(self, identifier, **kwargs):
            ...
            assert kwargs["approval_mode"] == ApprovalMode.deny_all and kwargs["sandbox"] == Sandbox.read_only
        ...
        async def stream(self):
            yield SimpleNamespace(method="item/agentMessage/delta", payload=SimpleNamespace(delta="Ready for review.", item_id="message"))
            yield SimpleNamespace(method="turn/completed", payload=TurnCompletedNotification.model_validate({"threadId": self.id,
                "turn": {"id": "turn", "status": "completed", "items": [], "error": None}}))
        async def interrupt(self):
            pass
    monkeypatch.setattr(openai_codex, "AsyncCodex", Codex)
    connection = {"id": "account", "revision": 1, "provider_id": "codex", "protocol": "codex", "auth_type": "client_login", "billing": "subscription"}
    route = {"model_id": "exact-model", "max_output_tokens": 1024}
    binding = {"id": "a" * 32, "run_id": "run", "connection_id": "account", "connection_revision": 1,
               "protocol": "codex", "model_id": "exact-model", "scope_fingerprint": "b" * 64,
               "settings_fingerprint": "c" * 64, "provider_session_id": Codex.id}
    context = SimpleNamespace(account_home=tmp_path, operation_id="run", bridge=Bridge(), control=Control(), session_binding=binding)
    result = await execute_codex(route, connection, {}, context, "Inspect the approved source.", object,
                                 before_request=lambda *args, **kwargs: "reservation", on_response=lambda *args: None)
    assert methods == [Codex.id] and result["usage"]["session_id"] == Codex.id
    assert any(kind == "assistant_text_delta" and data["text"] == "Ready for review." for kind, data in events)


def test_resumed_codex_usage_does_not_charge_prior_thread_tokens_again():
    last = SimpleNamespace(input_tokens=5, output_tokens=2, cached_input_tokens=3, reasoning_output_tokens=1, total_tokens=7)
    total = SimpleNamespace(input_tokens=1000, output_tokens=500, cached_input_tokens=500, reasoning_output_tokens=10, total_tokens=1500)
    usage = {}
...
def test_grok_resume_never_matches_another_sessions_title(tmp_path):
    ...
    with pytest.raises(ValueError, match="UUID"):
        headless_command(["grok"], route, 2, tmp_path / "agent.md", tmp_path / "prompt", resume_session_id="Another Tender")
    identifier = "12345678-1234-1234-1234-123456789abc"
    command = headless_command(["grok"], route, 2, tmp_path / "agent.md", tmp_path / "prompt", resume_session_id=identifier)
