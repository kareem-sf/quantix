...
def test_direct_profiles_derive_billing_and_never_keep_local_escape(tmp_path):
    ...
    saved = [service.create(_connection(provider)) for provider in (
        "openai", "anthropic", "google", "xai", "custom"
    )]
...
def test_unknown_price_meter_requires_explicit_consent_and_keeps_cost_null(tmp_path):
    ...
    asyncio.run(accepted.on_response({
        "usage_complete": True,
        "input_tokens": 100,
        "output_tokens": 20,
        "requests": 1,
...
def test_failed_direct_check_keeps_a_bounded_retry_available(tmp_path, monkeypatch):
    ...
    service = AIConnectionService(repo)
    service.save_model(connection["id"], {
        "model_id": "gpt-test",
    ...
    monkeypatch.setattr(direct, "direct_runtime_status", lambda _connection: {
...
class _SubscriptionComponents:
    def status(self, _connection):
        return {"component_id": "client-grok", "state": "ready", "detail": "ready", "progress": 100, "version": "client-test"}
...
def test_subscription_preview_uses_nullable_caps_and_ignores_fetched_timestamp(tmp_path):
    ...
    connection = connections.create({
        "name": "Grok subscription", "provider_id": "grok_build", "protocol": "grok_build",
        "auth_type": "client_login", "billing": "subscription", "settings": {},
    })
    connections.save_model(connection["id"], {
        "model_id": "grok-4.5", "display_name": "Grok 4.5",
        "capabilities": {"tools": True, "structured_output": True},
    })
    from quantix.ai_setup import AISetupService
    setup = AISetupService(repo, components=_SubscriptionComponents(), worker=_SubscriptionWorker(), direct=object())
    current = datetime.now(UTC)
    usage = {
        "fetched_at": current.isoformat(), "subscription_tier": "SuperGrok", "used_percent": 10,
        "period_type": "weekly", "period_start": (current - timedelta(minutes=1)).isoformat(),
        "period_end": (current + timedelta(hours=1)).isoformat(), "prepaid_balance_usd": "0",
        "on_demand_cap_usd": "0", "on_demand_used_usd": "0", "auto_topup_enabled": False,
        "included_only_allowed": True, "detail": "Included allowance confirmed.",
    }
    setup.store.update(connection["id"], stage="ready_to_check", selected_model_id="grok-4.5", subscription_usage=usage)
    first = setup.check_preview(connection["id"])
    setup.store.update(connection["id"], subscription_usage={**usage, "fetched_at": (current + timedelta(seconds=2)).isoformat()})
...
def test_grok_extras_preference_is_a_revisioned_subscription_only_action(tmp_path):
    ...
    result = asyncio.run(setup.action(connection["id"], {"action": "set_subscription_extras", "allow_provider_managed_extras": True}))
