...
def _line(**values):
    return TakeoffLineProposal.model_validate({
        "description": "Foundation concrete", "unit": "m3", "quantity": "12.5", "method": "dimensions",
        "working": "6 nr x 2.0 x 2.0 x 0.5 = 12.0; plus ground beam 0.5", "source_ids": ["drawing"], **values})
...
def test_missing_and_unshown_items_and_unknown_boq_quantities():
    ...
    assert compare(_line(quantity=None, method=None, unit="m2", boq_item_id="item"), item)["comparison"] == "not_on_drawings"
    assert compare(_line(boq_item_id="item"), item | {"unit": "m3", "supplied_quantity": None})["comparison"] == "no_boq_quantity"
    assert normal_unit(" No. ") == "nr" and normal_unit("م²") == "m2"
    with pytest.raises(ValueError):
        TakeoffLineProposal.model_validate({"description": "Nothing", "unit": "m", "working": "x", "source_ids": ["s"]})
...
def office(tmp_path, monkeypatch):
    ...
    drawing, _ = repo.register_artifact(
        tender["id"], "Drawings/S-101 Foundations.pdf", hashlib.sha256(text.encode()).hexdigest(), len(text),
...
async def test_a_boq_item_must_be_read_before_a_line_is_matched_to_it(office):
    ...
    line = {"description": "Foundation concrete", "unit": "m3", "quantity": "12.5", "method": "dimensions",
            "working": "6 x 2.0 x 2.0 x 0.5 + 0.5", "source_ids": [drawing_source], "boq_item_id": item["id"]}
    with pytest.raises(ToolArgumentError, match="inspect_estimate"):
        await _tool("propose").invoke(context, {"kind": "takeoff", "items": [line]})


@pytest.mark.asyncio
async def test_staff_takeoff_is_saved_on_completion_and_reviewed_by_the_engineer(office, monkeypatch):
    ...
    surveyor = team.hire(tender["id"], run["id"], StaffDraft(
        name="Samir Haddad", role="Quantity Surveyor", specialisms=["Substructure takeoff"],
        background="Twenty years measuring civil works.", working_style="Measures from dimensions first."))
    assignment = team.assign(tender["id"], run["id"], surveyor.id, title="Foundations takeoff",
                             brief="Take off drawing S-101 and check it against the BOQ.",
                             expected_result="Takeoff lines", source_ids=[drawing_source], route=route)
    ...
    async def provider(route, connection, credentials, context, prompt, output_type, **kwargs):
        ...
        await dispatch("direct", tools["inspect_estimate"], context, {"offset": 0, "limit": 10}, invocation_id="s-boq")
        await dispatch("direct", tools["read_source"], context, {"source_id": drawing_source}, invocation_id="s-read")
        await dispatch("direct", tools["propose"], context, {"kind": "takeoff", "items": [
            {"description": "Pad footings and ground beam concrete", "location": "Grid A-C/1-3", "unit": "m3",
             "quantity": "12.5", "method": "schedule", "working": "6 x 2.0 x 2.0 x 0.5 = 12.0; ground beam 0.5",
             "source_ids": [drawing_source], "boq_item_id": item["id"]},
            {"description": "Precast manholes", "unit": "nr", "quantity": "3", "method": "schedule",
             "working": "3 nr from the foundation schedule", "source_ids": [drawing_source]},
        ]}, invocation_id="s-propose")
        return {"output": {"kind": "completed", "summary": "Two takeoff lines saved; manholes are not in the BOQ.",
                           "source_ids": [drawing_source]},
                "usage": {"requests": 3}, "web_sources": []}
    ...
    with TestClient(app) as client:
        ...
        reviewed = client.post(f"/api/tenders/{tender['id']}/takeoff/{missing}/review",
                               json={"decision": "accepted", "note": "Raise as a clarification."})
        assert reviewed.status_code == 200, reviewed.text
        assert reviewed.json()["status"] == "accepted"
    with repo.db.connect() as conn:
        decision = conn.execute("SELECT decision,rationale FROM decisions WHERE target_type='takeoff_line' AND target_id=?",
                                (missing,)).fetchone()
    assert tuple(decision) == ("accepted", "Raise as a clarification.")
    listed = json.loads(await _tool("inspect_tender_records").invoke(
        OfficeContext(repo, tender["id"], run["id"]), {"record_type": "takeoff", "offset": 0, "limit": 10}))
