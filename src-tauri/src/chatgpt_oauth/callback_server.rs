...
pub(crate) enum CallbackFailure {
    PortBlocked(PortHolders),
    ProviderDenied(String),
    StateMismatch,
    ExchangeFailed(ExchangeFailure),
    Startup(&'static str),
    Timeout,
}
...
impl std::fmt::Display for CallbackFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CallbackFailure::PortBlocked(_) => {
                write!(f, "callback ports are held by other processes")
            }
            CallbackFailure::ProviderDenied(reason) => write!(f, "sign-in was denied: {reason}"),
            CallbackFailure::StateMismatch => write!(f, "callback state did not match"),
            CallbackFailure::ExchangeFailed(failure) => match failure {
                ExchangeFailure::Rejected(kind) => {
                    write!(f, "token exchange rejected ({kind:?})")
                }
                ExchangeFailure::Unreachable => write!(f, "token service unreachable"),
            },
            CallbackFailure::Startup(detail) => write!(f, "sign-in could not start: {detail}"),
            CallbackFailure::Timeout => write!(f, "sign-in timed out"),
        }
    }
}
...
pub(crate) fn run_login(
    ...
    let Some((listener, port)) = bind_first_free(port_candidates) else {
        return CallbackOutcome::Failed(CallbackFailure::PortBlocked(resolve_holders(
            port_candidates,
        )));
    };
...
fn serve_until_terminal(
    ...
    match rx.recv_timeout(budget + SEND_GUARD) {
        ...
        Err(RecvTimeoutError::Disconnected) => CallbackOutcome::Failed(CallbackFailure::Startup(
            "login server stopped unexpectedly",
        )),
    }
}
...
fn handle_callback(
    ...
    if let Some(error) = param(&params, "error") {
        ...
        return CallbackOutcome::Failed(CallbackFailure::ProviderDenied(reason));
    }
    ...
    match client.exchange_code(&code, &pending.redirect_uri, &pending.pkce) {
        ...
        Err(TokenError::Provider { kind, .. }) => {
            ...
            CallbackOutcome::Failed(CallbackFailure::ExchangeFailed(ExchangeFailure::Rejected(
                kind,
            )))
        }
        Err(TokenError::Transport(_)) => {
            ...
            CallbackOutcome::Failed(CallbackFailure::ExchangeFailed(
...
mod tests {
    ...
    fn provider_error_param_maps_to_denied_failure_and_error_page() {
        ...
        match harness
            ...
            .expect("terminal outcome")
        {
            CallbackOutcome::Failed(CallbackFailure::ProviderDenied(message)) => {
                assert_eq!(message, "User denied the request");
            }
    ...
    fn blocked_both_candidates_reports_holder_pids() {
        ...
        match harness
            ...
            .expect("terminal outcome")
        {
            CallbackOutcome::Failed(CallbackFailure::PortBlocked(holders)) => {
