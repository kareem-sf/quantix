...
Generic agent frameworks such as PydanticAI, the OpenAI Agents SDK, LangGraph and CrewAI are useful design references, but Quantix v0 does not install one. They introduce another runtime and state model and would duplicate the bounded provider execution already owned by Quantix. The genuine Rust Quantix Host owns the ChatGPT OAuth connection, direct HTTPS/SSE execution, Typed Tool mediation and the deterministic Tender Office control plane.
...
Each specialist is a separate versioned Agent Profile with bounded Provider Turns, exact tasks, inputs, permissions, output contracts and independent review.
...
Agent runtime	Quantix-owned ChatGPT OAuth and direct HTTPS/SSE adapter	The Rust Host owns the subscription connection, bounded Provider Turns and host-authorized Typed Tools behind the AI Provider Interface; no OpenAI API key, routing or silent fallback.
  ...
  apps/
    desktop/
      ...
      src-tauri/
        ...
        binaries/          # target-specific local document-runtime sidecars
  crates/
    domain/
    workflow/
    agents/
...
the Rust Host owns narrow direct AI Provider adapters, while the Rust Process Supervisor owns replaceable local document-runtime processes.

There should be no microservices, Kubernetes, cloud collaboration server, event broker or distributed database in the first product. One Tauri Rust Host, local SQLite Tender Stores, a content-addressed file store, direct bounded provider calls and disposable OCR jobs are sufficient.
...
Quantix-owned ChatGPT browser OAuth and direct HTTPS/SSE Provider Adapter.
...
Product acceptance is layered. Private v0 Qualification requires the complete deterministic suite, five consecutive clean live runs of the synthetic bilingual Acceptance Tender Fixture against the versioned direct ChatGPT adapter and catalogue, and a packaged Windows 11 end-to-end rehearsal. Public Release Gate additionally requires equivalent native packaged acceptance on Windows 11 x64, macOS 14+ Apple Silicon and Ubuntu 24.04 x64, plus production assurance and terms permitting the intended third-party subscription-backed ChatGPT integration.
...
Product qualification	Private Windows engineer-operated v0 first; public support requires native Windows, macOS and Linux qualification plus a supported and permitted direct ChatGPT integration
