...
import { MenuIcon } from "@/components/ui/animated-icons"
...
type SidebarContextProps = {
  ...
  isMobile: boolean
  toggleSidebar: () => void
  /** A collapsed sidebar is showing its full contents while the pointer rests on it. */
  peek: boolean
...
function SidebarProvider({
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    () => ({
      ...
      setOpenMobile,
      toggleSidebar,
      peek: peek && state === "collapsed" && !isMobile,
      setPeek,
    }),
    [
      state,
...
function Sidebar({
  ...
  // While the pointer rests on a collapsed sidebar it shows its full contents
  // over the page. The gap keeps the collapsed width so nothing else moves.
  const collapsed = state === "collapsed"
  const showsLabels = !collapsed || peek

  return (
    <div
      className="group peer hidden text-sidebar-foreground md:block"
      data-state={showsLabels ? "expanded" : "collapsed"}
      data-collapsible={showsLabels ? "" : collapsible}
      data-gap={collapsed ? collapsible : ""}
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          "group-data-[gap=offcanvas]:w-0",
          "group-data-[side=right]:rotate-180",
          variant === "floating" || variant === "inset"
            ? "group-data-[gap=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4)))]"
            : "group-data-[gap=icon]:w-(--sidebar-width-icon)"
        )}
      />
      <div
        ref={containerRef}
        ...
        data-side={side}
        onMouseEnter={
          collapsible === "icon" && collapsed ? () => setPeek(true) : undefined
        }
        onMouseLeave={
          collapsible === "icon"
            ? (event) => {
                // An open menu or a focused control inside keeps the sidebar
                // open, so moving to its popup does not close it.
      ...
      >
        <div
          ...
          className={cn(
            ...
            "group-data-[peek=true]:rounded-xl group-data-[peek=true]:shadow-2xl group-data-[peek=true]:ring-1 group-data-[peek=true]:ring-sidebar-border"
          )}
        >
          {children}
        </div>
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      {/* Animated icon adapted from Skiper UI (skiper99). */}
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ms-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[gap=icon]:ms-2",
        className
      )}
      {...props}
    />
