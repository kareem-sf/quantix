...
const signInPages: Partial<
  Record<Schema<"ConnectionRecord">["protocol"], string>
> = {
  grok_build: "https://grok.com/",
};

export async function prepareSignInBrowser(
  protocol: Schema<"ConnectionRecord">["protocol"],
) {
  ...
  try {
    ...
    throw new Error(
      typeof error === "string"
        ? error
        : "Your default browser could not open. Reopen Quantix to finish updating, then try sign-in again.",
    );
  }
}
