...
def test_items_are_saved_only_when_the_page_backs_them(client, package, qs):
    ...
    assert client.get(f"/tenders/{tender_id}/gates").json() == {"boq": 2, "facts": 0, "takeoff": 0, "pricing": 0}
...
def test_an_autonomous_office_approves_its_own_items(client, package, qs):
    ...
    assert client.get(f"/tenders/{tender_id}/gates").json() == {"boq": 0, "facts": 0, "takeoff": 0, "pricing": 0}
