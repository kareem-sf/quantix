...
- **Blocking**: GitHub's native issue dependencies. Add an edge with `gh api --method POST repos/<owner>/<repo>/issues/<child>/dependencies/blocked_by -F issue_id=<blocker-db-id>`.
- **Frontier query**: list the map's open children, remove blocked or assigned tickets, and select the first in map order.
- **Claim**: `gh issue edit <n> --add-assignee @me`.
- **Resolve**: comment on and close the ticket, then append a context pointer to the map's Decisions-so-far.
