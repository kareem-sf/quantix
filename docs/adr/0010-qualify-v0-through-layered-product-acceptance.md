# Qualify v0 through layered product acceptance
...
release candidate against deterministic checks, the original bilingual
Acceptance Tender Fixture, repeated live runs through the direct ChatGPT
provider adapter, and a packaged end-to-end rehearsal. Public Release Gate adds
native packaged qualification on every supported platform and blocks
distribution until applicable OpenAI terms and product authorization support
Quantix's intended subscription-backed integration.

## Consequences

- One fully synthetic bilingual Egyptian employer-designed building Tender, synthetic contractor library, and machine-readable Acceptance Oracle remain the public golden fixture. Authentic FIDIC text and real-company material remain in a separate private licensed calibration lane.
- Every pull request runs deterministic domain, calculation, document, evidence,
  permission, prompt-injection, file-security, storage, recovery,
  provider-contract, artifact, UI, and regression checks without requiring a
  live account. Deterministic direct-provider fixtures and scripted SSE streams
...
- A release candidate must complete five consecutive clean live runs through
  ...
  calculations; attach an approved provenance class to 100% of material claims;
  and recall at least 95% of non-critical oracle items.
- No averaging can hide a critical, calculation, evidence, permission, EITL, information-boundary, prompt-injection, integrity, recovery, or security failure. Any such failure blocks qualification. Tender package Critical Review Findings remain non-waivable; permitted Major findings require exact EITL Exception Approval, and disclosed Minor findings may remain only through Final Approval.
- Private v0 Qualification requires the full native packaged lifecycle on
  Windows 11 x64 with an opted-in Engineer User's Quantix-owned ChatGPT OAuth
  connection. It is explicitly non-public and makes no production-support
  claim.
- Public Release Gate requires the same signed-package acceptance on Windows 11
  x64, macOS 14+ Apple Silicon, and Ubuntu 24.04 x64. Native CI runs the
  deterministic suite on all three; a private live-provider qualification and
  native accessibility, updater, installation, import, processing, recovery,
  export, and uninstall checks must pass on each target before support is
  claimed.
- Product acceptance enforces versioned Safety Limits, timeouts, bounded resources, free-disk checks, no hangs, no orphaned child processes, and no partial canonical publication. v0 records processing time by stage but invents no latency SLA before a complete fixture establishes a measured baseline; later releases may set regression limits from that evidence.
...
- [Decision ticket](https://github.com/kareem-sf/quantix/issues/14)
