...
ADR 0014 supersedes this decision's application-wide runtime-selection scope. Application Settings now supplies a default for new Tenders, while each Tender owns the exact selection used by its future Agent Runs. The remaining consequences below continue to apply.

## Consequences

- Quantix initially permits one Provider Connection for each supported provider. Several providers may be connected simultaneously. Application Settings holds one default Provider Connection, provider-qualified model, and provider-native reasoning setting that is copied into a new Tender; every existing Tender owns its selection independently.
...
- Every new Agent Run immutably captures its Tender's Provider Connection identity, provider-qualified model, provider-native reasoning selection, catalogue provenance, and adapter/runtime version. Changing the Tender selection affects only its future runs; active, queued, interrupted, and indeterminate runs retain their captured configuration.
...
- Connecting a provider presents a concise data-destination and cost-account disclosure. Application Settings shows the default for new Tenders; the active Tender selection and exact captured selection remain visible in the Manager composer and Agent Run details without repeated approval prompts for every ordinary turn.
