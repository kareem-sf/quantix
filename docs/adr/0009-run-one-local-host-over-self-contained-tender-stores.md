...
- A deep Process Supervisor Module owns Tokio tasks, bounded stdio, cancellation, timeouts, stderr limits, and terminal process facts. `process-wrap` supplies Windows Job Objects and Unix process-group/session containment. Codex and Docling protocol Adapters are internal seams; the renderer receives no Tauri shell-plugin permission and cannot choose executables or arguments.
...
- Official OpenAI documentation currently marks `codex app-server` experimental and unsupported for production. Existing-login integration may be developed and privately evaluated, but public release remains blocked until OpenAI provides production assurance or an explicit risk decision accepts the unsupported dependency.
...
- The minimum intended implementation stack is Tauri 2, React, TypeScript, Vite, Tokio, `process-wrap`, `rusqlite`, `cacache`, Rust `zip`, Serde, `serde_json`, `serde_json_canonicalizer`, `sha2`, `garde`, `ts-rs`, `jsonschema`, `tracing`, `tempfile`, `thiserror`, pinned `uv`, and Docling. Exact versions and features belong in lockfiles and are checked against current documentation before implementation.
